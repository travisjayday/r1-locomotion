...
class _MockState:
    ...
    def __init__(self, num_envs: int, num_dofs: int, num_bodies: int, anchor_idx: int):
        ...
        self.dof_forces = torch.randn(num_envs, num_dofs)
...
class _MockRefState:
    ...
    def __init__(self, num_envs: int, num_bodies: int):
        ...
        self.rigid_body_pos = torch.randn(num_envs, num_bodies, 3)
        self.rigid_body_contacts = torch.zeros(
...
class _MockMimic:
    ...
    def __init__(
        ...
        # Some tracker experiments expose a deliberately compact command view
        # instead of the generic future_* tensors. Keep the clean future values
        # as the export samples; training-only command perturbations are disabled
        # by the inference config.
        self.command_root_pos = self.future_anchor_pos
        self.command_root_rot = self.future_anchor_rot
        self.command_dof_pos = self.future_dof_pos
        self.command_dof_vel = self.future_dof_vel
...
class _MockSteering:
    """Mock deployable velocity command for steering-policy export."""

    def __init__(self, num_envs: int):
        import torch

        self.tar_local_vel = torch.randn(num_envs, 2)
        self.tar_yaw_rate = torch.randn(num_envs)
...
class MockContext:
    ...
    def __init__(
        ...
        self.body_contacts = torch.zeros(
            num_envs, num_contact_bodies, dtype=torch.bool
        )
        self.current_contact_force_magnitudes = torch.zeros(num_envs, num_bodies)
...
def export_tracker(
    ...
    decimation_override: int | None = None,
) -> Path:
    ...
    num_contact_bodies = len(robot_config.contact_bodies)
    body_names = list(robot_config.kinematic_info.body_names)
    contact_body_names = list(robot_config.contact_bodies)
    ...
    if decimation_override is not None:
        if decimation_override <= 0:
            raise ValueError("decimation_override must be positive")
        decimation = decimation_override
    if physics_fps is not None or decimation_override is not None:
        control_dt = physics_dt * decimation
    ...
    if validate:
        ...
        _enabled = _g(_acfg_v, "action_enabled")
        if _scale is not None and _offset is not None:
            ...
            if _enabled is not None:
                _a = _a * np.asarray(_enabled, dtype=np.float64).reshape(-1)
            _recon = _o + _s * _a
    ...
    action_enabled_vals = _as_float_list(_get(_acfg, "action_enabled"))
    ...
    yaml_content = _build_yaml(
        ...
        action_enabled=action_enabled_vals,
...
def _build_yaml(
    *,
    ...
    for onnx_name in onnx_in_names:
        ...
        if key == "body_contacts":
            entry["kind"] = "body_contacts"
            entry["element_names"] = [contact_body_names]
        elif key == "current.dof_forces":
            entry["kind"] = "joint_forces"
            entry["element_names"] = [joint_names]
        elif key == "current.rigid_body_pos":
            entry["kind"] = "body_pos"
            entry["element_names"] = [body_names, ["x", "y", "z"]]
        elif key == "current.rigid_body_rot":
            entry["kind"] = "body_rot"
            entry["element_names"] = [body_names, ["x", "y", "z", "w"]]
        elif key == "current.rigid_body_vel":
            entry["kind"] = "body_vel"
            entry["element_names"] = [body_names, ["x", "y", "z"]]
        elif key == "current.rigid_body_ang_vel":
            entry["kind"] = "body_ang_vel"
            entry["element_names"] = [body_names, ["x", "y", "z"]]
        elif key == "current.root_pos":
            entry["kind"] = "root_pos"
            entry["element_names"] = [["x", "y", "z"]]
        elif key == "current_contact_force_magnitudes":
            entry["kind"] = "contact_force_magnitudes"
            entry["element_names"] = [body_names]
        elif key == "ground_heights":
            entry["kind"] = "ground_height"
        elif key == "mimic.command_root_pos":
            entry["kind"] = "reference_motion_root_pos_future"
            entry["future_steps"] = len(future_step_indices)
            entry["element_names"] = [[anchor_body_name], ["x", "y", "z"]]
        elif key == "mimic.command_root_rot":
            entry["kind"] = "reference_motion_root_rot_future"
            entry["future_steps"] = len(future_step_indices)
            entry["element_names"] = [[anchor_body_name], ["x", "y", "z", "w"]]
        elif key == "mimic.ref_state.rigid_body_contacts":
            entry["kind"] = "reference_motion_body_contacts"
            entry["element_names"] = [body_names]
        elif "dof_pos" in key and "future" not in key and "command" not in key:
            entry["kind"] = "joint_pos"
            entry["element_names"] = [joint_names]
        elif "dof_vel" in key and "future" not in key and "command" not in key:
            ...
            entry["future_steps"] = len(future_step_indices)
            entry["element_names"] = [[anchor_body_name], ["x", "y", "z", "w"]]
        elif "mimic" in key and ("dof_pos" in key or "command_dof_pos" in key):
            entry["kind"] = "reference_motion_joint_pos"
            entry["future_steps"] = len(future_step_indices)
            entry["element_names"] = [joint_names]
        elif "mimic" in key and ("dof_vel" in key or "command_dof_vel" in key):
    ...
    content = {
        ...
        "control": {
            ...
            #     joint_pos_targets = pd_action_offset
            #                         + action_scale * action_enabled * action
...
def _parse_args():
    ...
    p.add_argument(
        ...
        type=float,
        default=None,
        help="Override MuJoCo physics frequency recorded in the deployment sidecar",
    )
    p.add_argument(
        "--decimation",
        type=int,
        default=None,
        help="Override physics substeps per policy step in the deployment sidecar",
    )
    return p.parse_args()
...
if __name__ == "__main__":
    ...
    onnx_file = export_tracker(
        ...
        decimation_override=args.decimation,
    )
