...
def _patch_mjcf_xml(xml_path: Path) -> str:
    ...
    if worldbody is not None:
        ...
        for g in worldbody.findall("geom"):
            ...
            geom_type = g.get("type", "").lower()
            if "floor" in name or "ground" in name or geom_type == "plane":
                if g.get("contype", "1") == "0" and g.get("conaffinity", "1") == "0":
                    # Decorative-only floor (e.g. r1.xml's IsaacLab reference
                    # grid) -- give it real collision instead of leaving the
                    # robot to fall through an uncollidable plane.
                    g.set("contype", "1")
                    g.set("conaffinity", "1")
                has_ground = True
                break
