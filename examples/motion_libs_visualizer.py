...
parser.add_argument(
    "--robot",
    type=str,
    choices=["g1", "r1", "h1_2", "smpl", "soma23"],
    default="g1",
...
parser.add_argument(
    "--runtime_pose_qa",
    action="store_true",
    help="Print target versus post-PhysX root/foot positions every 30 steps",
)
parser.add_argument(
    "--fixed_camera",
    action="store_true",
    help="Do not continuously move the IsaacLab viewport with the robot",
)
parser.add_argument(
    "--origin_xy",
    type=float,
...
class MotionVisualizerSmoothness:
    def __init__(
        ...
        # The visualizer writes an exact kinematic pose before every rendered
        # frame. Per-body disable_gravity is not sufficient on all IsaacLab 3
        # backends: the decimated physics step can still translate the entire
        # free articulation before rendering. Disable scene gravity explicitly
        # so post-step body poses remain aligned with the MotionLib target.
        self.simulator_cfg.sim.gravity = (0.0, 0.0, 0.0)
        ...
        if args.simulator == "isaaclab":
            self.simulator.camera_follow_enabled = not args.fixed_camera
        ...
        print("  'F' - Toggle camera following (IsaacLab)")
    ...
    def _set_robot_pose(self, dof_pos, rigid_body_pos=None, rigid_body_rot=None):
        ...
        self._runtime_qa_target_body_pos = rigid_body_pos.detach().clone()
        self._runtime_qa_target_dof_pos = dof_pos.detach().clone()
    ...
    def _print_runtime_pose_qa(self, step_count: int) -> None:
        """Compare the displayed post-step state with the MotionLib target."""
        if not args.runtime_pose_qa or step_count % 30 != 0:
            return
        if not hasattr(self, "_runtime_qa_target_body_pos"):
            return

        actual = self.simulator.get_robot_state()
        target_pos = self._runtime_qa_target_body_pos
        body_error = torch.linalg.norm(actual.rigid_body_pos - target_pos, dim=-1)
        target_dof = self._runtime_qa_target_dof_pos
        dof_error = torch.abs(actual.dof_pos - target_dof)

        names = [self.kinematic_info.body_names[0]]
        for group in ("all_left_foot_bodies", "all_right_foot_bodies"):
            names.extend(self.robot_cfg.common_naming_to_robot_body_names[group])
        fields = []
        for name in names:
            index = self.kinematic_info.body_names.index(name)
            fields.append(
                f"{name}: target_z={target_pos[0, index, 2].item():.4f}, "
                f"actual_z={actual.rigid_body_pos[0, index, 2].item():.4f}, "
                f"err={body_error[0, index].item():.4f}m"
            )
        print(
            "runtime pose QA | " + " | ".join(fields)
            + f" | max_body_err={body_error.max().item():.4f}m"
            + f" | max_dof_err={dof_error.max().item():.4f}rad"
        )
    ...
    def toggle_camera_follow(self):
        """Toggle ProtoMotions' automatic IsaacLab viewport tracking."""
        if args.simulator != "isaaclab":
            print("Camera-follow toggle is currently available for IsaacLab only")
            return
        enabled = not self.simulator.camera_follow_enabled
        self.simulator.camera_follow_enabled = enabled
        if enabled:
            # Reset the camera reference so re-enabling follow does not apply
            # the robot's entire accumulated displacement as one jump.
            self.simulator._init_camera()
        print(f"Camera following {'enabled' if enabled else 'disabled'}")
    ...
    def run(self):
        ...
        while True:
            ...
            self._print_runtime_pose_qa(step_count)
...
def main():
    ...
    if args.simulator == "isaaclab":
        ...
        # Recent IsaacLab AppLauncher versions default to headless when no
        # visualizer intent is supplied, even if ``headless`` is False.
        # Explicitly select Kit so the native window, viewport, and carb.input
        # keyboard extension are loaded for interactive playback.
        if not args.headless:
            app_launcher_flags.update(
