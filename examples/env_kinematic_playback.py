...
def create_parser():
    ...
    parser.add_argument(
        ...
        type=float,
        default=1.0,
        help="Wall-clock playback speed (1.0 is real time, 0.5 is half speed)",
    )
...
def main():
    ...
    if args.playback_speed <= 0:
        parser.error("--playback-speed must be greater than zero")
    ...
    sim_step_dt = env.simulator.decimation / env.simulator.config.sim.fps
    target_wall_dt = sim_step_dt / args.playback_speed
    ...
    try:
        ...
        while env.is_simulation_running():
            ...
            # MuJoCo and other CPU backends otherwise replay as fast as they can
            # compute. Keep simulation time synchronized to the requested
            # wall-clock playback rate without changing the motion trajectory.
            remaining = target_wall_dt - (time.perf_counter() - frame_start)
            if remaining > 0:
