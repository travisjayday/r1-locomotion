...
"""Viser-based visualizer for SMPL, SOMA23, and R1 motion files.
...
SKELETONS = {
    ...
    "r1": {
        "bodies": [
            "pelvis_link", "left_hip_pitch_link", "left_hip_roll_link",
            ...
            "right_ankle_pitch_link", "right_ankle_roll_link", "waist_roll_link",
            "waist_yaw_link", "left_shoulder_pitch_link",
            ...
            "right_wrist_roll_link", "head_pitch_link", "head_yaw_link",
        ],
        "parents": [-1, 0, 1, 2, 3, 4, 5, 0, 7, 8, 9, 10, 11, 0, 13,
                    14, 15, 16, 17, 18, 14, 20, 21, 22, 23, 14, 25],
        "regions": {
            "spine": [0, 13, 14, 25, 26],
            "left_arm": [15, 16, 17, 18, 19],
            "right_arm": [20, 21, 22, 23, 24],
            "left_leg": [1, 2, 3, 4, 5, 6],
            "right_leg": [7, 8, 9, 10, 11, 12],
        },
    },
}
...
def load_motion(path: str) -> dict:
    ...
    elif "rigid_body_pos" in data and data["rigid_body_pos"].dim() == 3:
        # ProtoMotions common .motion format (RobotState field names).
        return {
            "motions": [
                {
                    "gts": data["rigid_body_pos"].numpy(),
                    "grs": data["rigid_body_rot"].numpy(),
...
def main():
    ...
    parser.add_argument(
        "--robot",
        default=None,
        help="Robot type (smpl/soma23/r1). Auto-detected if omitted.",
    )
    parser.add_argument(
        "--compare",
        default=None,
    ...
    def body_meshes_for(skeleton_name, body_regions):
        if skeleton_name == "smpl":
            return _build_body_meshes(SMPL_GEOM_DEFS, body_regions)
        meshes = {}
        for body_idx in range(len(SKELETONS[skeleton_name]["bodies"])):
            mesh = trimesh.creation.icosphere(subdivisions=1, radius=0.035)
            region = body_regions.get(body_idx, "spine")
            mesh.visual.face_colors = REGION_COLORS[region] + [220]
            meshes[body_idx] = mesh
        return meshes

    body_meshes = body_meshes_for(robot, body_to_region)
    cmp_body_meshes = (
        body_meshes_for(cmp_robot, cmp_body_to_region)
    ...
    def render_skeleton(
        ...
        segments = []
        for child, parent in enumerate(skel_info["parents"]):
            if parent >= 0:
                p0, p1 = gts[parent].copy(), gts[child].copy()
                p0[0] += x_offset
                p1[0] += x_offset
                segments.append([p0, p1])
        bone_name = f"{prefix}bones"
        if bone_name in handles_bone:
            handles_bone[bone_name].remove()
        if segments:
            color = (80, 80, 80) if not prefix else (220, 80, 80)
            handles_bone[bone_name] = server.scene.add_line_segments(
                f"/skel/{bone_name}",
    ...
    while True:
        ...
        if cmp_motions is not None:
            ...
            primary_fps = float(motion["fps"])
            compare_fps = float(cmp["fps"])
            cmp_fi = min(
                round((fi / primary_fps) * compare_fps),
                cmp["gts"].shape[0] - 1,
            )
            cmp_contacts = (
