...
class ControlInfo:
    ...
    # Optional IsaacLab explicit-motor model.  These fields are ignored by
    # simulators which only support ideal/implicit PD actuators.
    actuator_model: Optional[str] = field(default=None)
    torque_speed_x1: Optional[float] = field(default=None)
    torque_speed_x2: Optional[float] = field(default=None)
    torque_same_direction: Optional[float] = field(default=None)
    torque_opposite_direction: Optional[float] = field(default=None)
    friction_static: Optional[float] = field(default=None)
    friction_dynamic: Optional[float] = field(default=None)
    friction_activation_velocity: Optional[float] = field(default=None)
            ...
            for (
                ...
                if re.fullmatch(joint_expr, joint_name):
                    ...
                    for field_name in (
                        ...
                        "torque_speed_x1",
                        "torque_speed_x2",
                        "torque_same_direction",
                        "torque_opposite_direction",
                        "friction_static",
                        "friction_dynamic",
                        "friction_activation_velocity",
                    ):
                        value = getattr(override_joint_control_info, field_name)
                        if value is not None:
                            explicit_motor_params[field_name] = value
...
def fk_from_transforms_with_velocities(
    ...
    # matrix_to_quaternion converts every frame independently (this whole
    # function operates on a batch of frames with no cross-frame state), so
    # it can pick either sign of the equivalent q/-q pair frame to frame.
    # Downstream consumers that already wrap/hemisphere-correct (angle
    # metrics through atan2, slerp) are unaffected either way, but this
    # keeps the stored trajectory itself visually and representationally
    # continuous -- naive per-frame rendering or raw linear interpolation
    # of a discontinuous sequence shows an apparent ~360-degree snap at the
    # sign flip even though the underlying rotation never actually jumped.
    world_quat = enforce_quaternion_hemisphere_continuity(world_quat)
