...
# Enable Ampere-and-newer TensorFloat32 tensor cores before any model is built
# or compiled. This is training-time TF32 acceleration; TensorRT is an inference
# runtime and is not involved in the PyTorch training loop.
...
def apply_resume_training_iteration_limit(max_iterations, agent_config):
    """Honor an explicitly supplied absolute iteration limit during resume."""
    if max_iterations is None:
        return
    agent_config.training_max_iterations = max_iterations
    log.info(
        "RESUME: overriding frozen training_max_iterations with explicit CLI "
        "value %d",
        max_iterations,
    )
...
def main():
    ...
    # detect_checkpoint_mode() restores the saved CLI namespace for a true
    # resume. Preserve this one explicitly mutable stopping condition first.
    requested_resume_max_iterations = args.training_max_iterations
    ...
    if mode == "resume":
        ...
        apply_resume_training_iteration_limit(
            requested_resume_max_iterations, agent_config
        )
        ...
        log.info(
            "RESUME: Using frozen configs from first run; an explicitly supplied "
            "training iteration limit remains overrideable"
        )
