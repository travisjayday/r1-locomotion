...
- **Z**: Quit
- **W/A/S/D**: Manual movement with a keyboard command source

Use ``--command-source steering=window`` for a dedicated steering panel whose
keys do not conflict with simulator viewer shortcuts.
...
def create_parser():
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "target=keyboard, steering=keyboard, or steering=window. "
            "The window source opens a dedicated steering panel that displays "
            "the live command. A bare value applies to the single control component."
        ),
    )
    parser.add_argument(
        "--max-steps",
        ...
        default=None,
        help=(
            "Stop interactive inference after this many policy steps and report "
            "the achieved loop rate. Omit to run until Ctrl+C."
        ),
    )
    parser.add_argument(
        ...
        default=None,
        help=(
            "IsaacLab viewport render scale in (0, 1]. Lower values trade "
            "image sharpness for higher interactive frame rate."
        ),
    )
    parser.add_argument(
        "--kit-args",
        ...
        default="",
        help=(
            "Raw Omniverse Kit arguments forwarded when using IsaacLab. Since "
            "Kit options begin with '--', use the equals form, for example "
            "--kit-args='--/rtx/example=value --/rtx/other=false'."
        ),
    )
    parser.add_argument(
        "--cpu-physics",
        ...
        default=True,
        help=(
            "Run IsaacLab physics (and the rest of the inference pipeline: env "
            "tensors and policy) on CPU instead of GPU. For small num_envs "
            "(e.g. interactive single-robot playback) IsaacLab's GPU/PhysX "
            "pipeline is dominated by fixed per-substep kernel-launch and "
            "device-sync overhead rather than actual compute, so CPU physics "
            "is substantially faster in wall-clock terms. The Kit viewport "
            "still renders on the GPU regardless of this setting. Only "
            "affects --simulator isaaclab; use --no-cpu-physics to keep the "
            "GPU/PhysX pipeline (e.g. for large num_envs)."
        ),
    )
    parser.add_argument(
        ...
        default=False,
        help=(
            "Disable per-frame camera-follow updates in the IsaacLab viewer. "
            "The camera is still positioned once at startup, but is not "
            "re-queried and re-aimed every frame, saving a root-state fetch "
            "per render call. Only relevant to interactive (non-headless) "
            "IsaacLab runs."
        ),
    )
...
def apply_command_source_overrides(env_config, command_source_specs):
    ...
    from protomotions.envs.control.steering_control import (
        KeyboardSteeringCommandSourceConfig,
        RandomSteeringCommandSourceConfig,
        RandomYawRateSteeringCommandSourceConfig,
        SteeringControlConfig,
        WindowSteeringCommandSourceConfig,
    )
    ...
    for spec in command_source_specs:
        if "=" in spec:
            ...
            compatible_components = [
                ...
                if isinstance(
                    component, (TargetControlConfig, SteeringControlConfig)
                )
            ]
            if len(compatible_components) != 1:
                raise ValueError(
                    "Bare --command-source values require exactly one compatible "
                    "target or steering control component"
                )
            component_name = compatible_components[0]
            source_name = spec
        ...
        if not isinstance(
            component_config, (TargetControlConfig, SteeringControlConfig)
        ):
            raise ValueError(
                f"Command source override '{component_name}={source_name}' only "
                "supports target or steering control components"
            )

        source_name = source_name.lower()
        if source_name in ("keyboard", "manual", "user", "user-control"):
            component_config.command_source = (
                KeyboardTargetCommandSourceConfig()
                if isinstance(component_config, TargetControlConfig)
                else KeyboardSteeringCommandSourceConfig()
            )
        elif source_name in ("window", "panel", "standalone"):
            if not isinstance(component_config, SteeringControlConfig):
                raise ValueError(
                    "The window command source currently supports steering "
                    "control components only"
                )
            component_config.command_source = WindowSteeringCommandSourceConfig()
        elif source_name in ("random", "training"):
            if isinstance(component_config, TargetControlConfig):
                component_config.command_source = RandomTargetCommandSourceConfig()
            elif component_config.yaw_rate_commands:
                component_config.command_source = (
                    RandomYawRateSteeringCommandSourceConfig()
                )
            else:
                component_config.command_source = RandomSteeringCommandSourceConfig()
        else:
            raise ValueError(
...
def apply_interactive_inference_optimizations(env_config, robot_config) -> None:
    """Remove simulator features used only by training diagnostics.

    Interactive inference does not consume rewards. Contact sensors may also
    be omitted when no observation component consumes contact data; contact-
    based terminations are removed in that case while non-contact fall checks
    remain active.
    """

    observation_components = getattr(env_config, "observation_components", {})
    control_components = getattr(env_config, "control_components", {})
    if any(
        type(getattr(component, "command_source", None)).__name__
        == "WindowSteeringCommandSourceConfig"
        for component in control_components.values()
    ):
        # The command panel already visualizes the command. Avoid two animated
        # USD point instancers (movement/facing arrows) in the Kit viewport.
        env_config._disable_visualization_markers = True

    def uses_contact_data(component) -> bool:
        get_bindings = getattr(component, "get_bindings_dict", None)
        if get_bindings is None:
            # Unknown/legacy components are kept conservative.
            return True
        has_contact_binding = any(
            "contact" in str(path).lower()
            for path in get_bindings().values()
        )
        if not has_contact_binding:
            return False
        get_params = getattr(component, "get_params", None)
        if get_params is not None and get_params().get("observe_contacts") is False:
            # Max-coordinate factories retain an empty contact tensor in their
            # function signature even when contacts are excluded from output.
            return False
        return True

    scene_obs = getattr(env_config, "scene_obs", None)
    scene_contacts_enabled = bool(getattr(scene_obs, "enabled", False))
    observations_need_contacts = scene_contacts_enabled or any(
        uses_contact_data(component)
        for component in observation_components.values()
    )
    if observations_need_contacts:
        return
    ...
    termination_components = getattr(env_config, "termination_components", {})
    removed_terminations = [
        name
        for name, component in termination_components.items()
        if uses_contact_data(component)
    ]
    for name in removed_terminations:
        termination_components.pop(name)

    log.info(
        "Interactive inference: disabled training-only contact sensors%s",
        (
            f" and contact terminations {removed_terminations}"
            if removed_terminations
            else ""
        ),
    )
...
def main():
    ...
    if args.viewport_resolution_scale is not None and not (
        0.0 < args.viewport_resolution_scale <= 1.0
    ):
        parser.error("--viewport-resolution-scale must be in (0, 1]")
    if args.kit_args and args.simulator != "isaaclab":
        parser.error("--kit-args is only supported with --simulator isaaclab")
    ...
    if args.viewport_resolution_scale is not None:
        log.info(
            "CLI override: viewport_resolution_scale = %s",
            args.viewport_resolution_scale,
        )
        simulator_config.viewport_resolution_scale = (
            args.viewport_resolution_scale
        )
    if args.disable_camera_follow:
        log.info("CLI override: disable_camera_follow = True")
        simulator_config.disable_camera_follow = True
    ...
    if not args.full_eval:
        apply_interactive_inference_optimizations(env_config, robot_config)

    motion_lib_config.validate()

    # Create fabric config for inference (simplified)
    # MuJoCo is CPU-only, so force CPU accelerator. IsaacLab defaults to CPU
    # physics too (see --cpu-physics): with small num_envs, PhysX-GPU is
    # dominated by fixed per-substep kernel-launch/sync overhead rather than
    # actual compute, so CPU is faster in wall-clock terms. This also moves
    # the rest of the pipeline (env tensors, policy) to CPU so tensors stay
    # on a single device; the Kit viewport still renders on the GPU.
    use_cpu_physics = args.simulator == "mujoco" or (
        args.simulator == "isaaclab" and args.cpu_physics
    )
    accelerator = "cpu" if use_cpu_physics else "gpu"
    if args.simulator == "isaaclab":
        log.info(f"CLI override: cpu_physics = {args.cpu_physics}")
    ...
    if args.simulator == "isaaclab":
        ...
        if args.kit_args:
            app_launcher_flags["kit_args"] = args.kit_args
    ...
    if not args.full_eval:
        actor_config = getattr(getattr(agent_config, "model", None), "actor", None)
        enable_interactive = getattr(env, "enable_interactive_inference", None)
        if enable_interactive is not None:
            enable_interactive(getattr(actor_config, "in_keys", ()))
    ...
    try:
        if args.full_eval:
            ...
            simple_test_kwargs = {"collect_metrics": True}
            if args.max_steps is not None:
                simple_test_kwargs["max_steps"] = args.max_steps
            agent.evaluator.simple_test_policy(**simple_test_kwargs)
    finally:
        # Ensure simulator viewer is properly closed (prevents hangs)
        if hasattr(env.simulator, "shutdown"):
            env.simulator.shutdown()
        elif hasattr(env.simulator, "close"):
            env.simulator.close()
