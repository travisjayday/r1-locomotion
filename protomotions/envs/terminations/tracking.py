...
def compute_anchor_heading_error_term(
    current_anchor_rot: Tensor,
    ref_rigid_body_rot: Tensor,
    anchor_idx: int,
    threshold: float = 0.5,
) -> Tensor:
    """Anchor heading (yaw) error termination.

    Complements compute_anchor_ori_error_term ("bad_ref_ori"), which compares
    projected-gravity Z and is yaw-invariant: spinning purely about the
    vertical axis doesn't change how "upright" something is, so a robot that
    stays perfectly upright but turns the wrong way goes completely
    undetected there. This checks heading directly.
    ...
    """
    ref_anchor_rot = ref_rigid_body_rot[:, anchor_idx, :]
    current_heading = calc_heading(current_anchor_rot, w_last=True)
    ref_heading = calc_heading(ref_anchor_rot, w_last=True)
    diff = current_heading - ref_heading
    wrapped = torch.atan2(torch.sin(diff), torch.cos(diff))
    return torch.abs(wrapped) > threshold
...
def compute_relative_body_pos_error_term(
    ...
    # Terminate if ANY tracked body exceeds threshold
