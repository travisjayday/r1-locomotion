...
def bm_pd_action(
    ...
    stiffness: Tensor,
    damping: Tensor,
    action_enabled: Tensor | None = None,
) -> Dict[str, Tensor]:
    ...
    if action_enabled is not None:
        # Disabled joints remain at their default PD target. Keeping the full
        # tensor layout preserves compatibility with existing motion files and
        # simulator DOF ordering.
        action = action * action_enabled
...
def make_bm_pd_action_config(
    robot_config,
    effort_fraction: float | None = None,
    disabled_dof_names: list[str] | None = None,
) -> Dict[str, Any]:
    """Create action config dict for BeyondMimic-style PD action scaling.

    Computes per-joint action scale as
    ``effort_fraction * effort_limit / stiffness``,
    ...
    Args:
        ...
        effort_fraction: Fraction of the static effort limit represented by a
            unit action. If omitted, uses the robot control configuration
            (1.0 for existing robots, 0.25 for R1).
    ...
    if effort_fraction is None:
        effort_fraction = robot_config.control.action_effort_fraction
    action_scale = effort_fraction * effort_limit / stiffness

    pd_action_offset = robot_config.default_dof_pos.clone()

    action_enabled = torch.ones(len(joint_names), dtype=torch.float32)
    if disabled_dof_names:
        unknown = sorted(set(disabled_dof_names) - set(joint_names))
        if unknown:
            raise ValueError(f"Unknown disabled DOF names: {unknown}")
        for name in disabled_dof_names:
            action_enabled[joint_names.index(name)] = 0.0
