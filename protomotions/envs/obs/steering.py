...
def compute_yaw_rate_steering_obs(
    tar_local_vel: Tensor,
    tar_yaw_rate: Tensor,
) -> Tensor:
    """Return deployable body-yaw-frame ``[vx, vy, yaw_rate]`` commands."""
    return torch.cat((tar_local_vel, tar_yaw_rate.unsqueeze(-1)), dim=-1)


__all__ = ["compute_steering_obs", "compute_yaw_rate_steering_obs"]
