...
def _clone_snapshot_value(value):
    """Clone nested tensor state without moving it off its current device."""
    if isinstance(value, Tensor):
        return value.clone()
    if isinstance(value, dict):
        return {key: _clone_snapshot_value(item) for key, item in value.items()}
    if isinstance(value, list):
        return [_clone_snapshot_value(item) for item in value]
    if isinstance(value, tuple):
        return tuple(_clone_snapshot_value(item) for item in value)
    return value
...
class BaseEnv:
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self._interactive_inference = False
        ...
        self.prev_rigid_body_vel = None

        # Per-body seconds since last contact, for feet-air-time rewards.
        # Initialized properly after simulator init when we know num_bodies
        self.contact_air_time = None
    ...
    def initialize_simulator(self):
        ...
        # Body velocities as of the end of the previous step. A touchdown
        # impact is only observable *before* the contact resolves it: by the
        # step a foot first reports contact its vertical velocity has already
        # been arrested, so the landing speed has to be read one step back.
        self.prev_rigid_body_vel = torch.zeros(
            self.num_envs, num_bodies, 3, dtype=torch.float, device=self.device
        )
        self.contact_air_time = torch.zeros(
            self.num_envs, num_bodies, dtype=torch.float, device=self.device
        )

        # Per-DOF effort limits, used to report torque headroom each step. The
        # simulator already returns applied_torque in the robot state, but it
        # was never reduced to a scalar, so a run could saturate its actuators
        # invisibly. Diagnosing r1_locomotion_11's sprint speed deficit needed
        # offline MuJoCo inverse dynamics purely because this was not logged.
        self.dof_effort_limits = None
        control = getattr(self.robot_config, "control", None)
        control_info = getattr(control, "control_info", None)
        dof_names = getattr(self.robot_config.kinematic_info, "dof_names", None)
        if control_info is not None and dof_names is not None:
            limits = [
                getattr(control_info[name], "effort_limit", 0.0) or 0.0
                for name in dof_names
                if name in control_info
            ]
            if len(limits) == len(dof_names) and min(limits) > 0.0:
                self.dof_effort_limits = torch.tensor(
                    limits, dtype=torch.float, device=self.device
                )

        if self.config.num_state_history_steps > 0:
            # Check if observation noise is configured - if so, allocate noisy buffers
            # Allocate noisy history for the largest configured envelope, even
            # when an epoch-0 curriculum stage has zero observation noise.
            active_dr = self.simulator.config.domain_randomization
            store_noisy = (
                active_dr is not None
                and active_dr.observation_noise is not None
                and active_dr.observation_noise.has_noise()
            )
    ...
    def enable_interactive_inference(self, actor_observation_keys) -> None:
        """Trim training-only work after all model modules are materialized.

        Agent setup needs critic and discriminator observations once to build
        their lazy layers and load a checkpoint. Interactive rollout only runs
        the actor, so subsequent environment steps can compute just its dynamic
        observation components and omit rewards/raw training diagnostics.
        """
        actor_keys = set(actor_observation_keys or ())
        if not actor_keys:
            return

        self.config.observation_components = {
            name: component
            for name, component in self.config.observation_components.items()
            if name in actor_keys
        }
        self._observation_buffer = {
            name: value
            for name, value in self._observation_buffer.items()
            if name in actor_keys
        }
        self._interactive_inference = True
    ...
    def get_markers_state(self):
        ...
        """
        if self.simulator.headless or getattr(
            self.config, "_disable_visualization_markers", False
        ):
            return {}
    ...
    def on_epoch_end(self, current_epoch: int):
        """Advance environment and control-component curricula.

        Args:
            current_epoch: Current epoch number
        """
        simulator_epoch_end = getattr(self.simulator, "on_epoch_end", None)
        if simulator_epoch_end is not None:
            simulator_epoch_end(current_epoch)
        self.control_manager.on_epoch_end(current_epoch)
    ...
    def post_physics_step(self):
        ...
        if self.state_history is not None:
            ...
            if self.state_history.store_noisy:
                active_dr = getattr(
                    self.simulator,
                    "active_domain_randomization_config",
                    self.simulator.config.domain_randomization,
                )
                obs_noise_cfg = active_dr.observation_noise
        ...
        if getattr(self, "_interactive_inference", False):
            self.rew_buf.zero_()
        else:
        ...
        dr_stage = getattr(
            self.simulator, "_domain_randomization_stage_metric", None
        )
        dr_intensity = getattr(
            self.simulator, "_domain_randomization_intensity_metric", None
        )
        if dr_stage is not None and dr_intensity is not None:
            self.extras["domain_randomization/stage"] = dr_stage
            self.extras["domain_randomization/intensity"] = dr_intensity
        ...
        dof_forces = getattr(rbs, "dof_forces", None)
        if self.dof_effort_limits is not None and dof_forces is not None:
            # Fraction of each actuator's effort limit currently being used.
            # utilization_max is the one to watch: a mean well under 1 hides
            # brief saturation on the one joint that matters, and saturation
            # is what caps a tracking policy's authority on fast motions.
            utilization = dof_forces.abs() / self.dof_effort_limits
            self.extras["torque/utilization_mean"] = utilization.mean()
            self.extras["torque/utilization_max"] = utilization.amax(dim=-1).mean()
            self.extras["torque/saturated_frac"] = (
                (utilization > 0.98).float().mean()
            )
            self.extras["torque/abs_mean_nm"] = dof_forces.abs().mean()
        ...
        self.prev_contact_force_magnitudes[:] = current_contact_force_magnitudes
        self.prev_rigid_body_vel[:] = rbs.rigid_body_vel

        # Update per-body air time for next step's feet-air-time reward: bodies
        # not in contact accumulate dt, bodies in contact reset to zero. This
        # buffer is one step behind by design (mirrors prev_contact_force_magnitudes
        # above) -- it is read by compute_reward() before this update runs, so it
        # always reflects air time accumulated through the end of the *previous* step.
        in_contact = current_contact_force_magnitudes > 1.0
        self.contact_air_time += self.dt
        self.contact_air_time[in_contact] = 0.0
    ...
    def _build_context(self, env_ids: Optional[Tensor] = None) -> EnvContext:
        ...
        ctx = EnvContext(
            ...
            # Two steps back, for the second-difference (oscillation) penalty.
            # Needs num_state_history_steps >= 2; None otherwise so the rest of
            # the env keeps working at the default depth of 1.
            prev2_processed_action=self._select_context_tensor(
            ...
            prev_rigid_body_vel=self._select_context_tensor(
                self.prev_rigid_body_vel, env_ids
            ),
            contact_air_time=self._select_context_tensor(
                self.contact_air_time, env_ids
            ),
            dt=self.dt,
    ...
    def reset(
        ...
        # Sample task commands before selecting reference states so command-
        # conditioned tasks can start from compatible motion frames.
        self.control_manager.before_env_reset(env_ids)
        ...
        self.prev_rigid_body_vel[env_ids] = 0.0
        self.contact_air_time[env_ids] = 0.0
    ...
    def _get_ref_reset_envs(
        ...
        if len(ref_env_ids) > 0:
            if not disable_motion_resample:
                # Record pre-reset failure outcomes (for adaptive bin sampling,
                # when enabled) before sample_motions overwrites motion_ids/times.
                record_bin_failures = getattr(self.motion_manager, "record_bin_failures", None)
                if record_bin_failures is not None:
                    record_bin_failures(ref_env_ids, self.terminate_buf[ref_env_ids])
                reference_reset = self.control_manager.sample_reference_reset(
                    ref_env_ids,
                    self.motion_manager,
                )
                if reference_reset is None:
                    ...
                    matched_motion_ids, matched_motion_times = reference_reset
                    if matched_motion_ids.shape != ref_env_ids.shape:
                        raise ValueError(
                            "matched motion IDs must have the same shape as ref_env_ids"
                        )
                    if matched_motion_times.shape != ref_env_ids.shape:
                        raise ValueError(
                            "matched motion times must have the same shape as ref_env_ids"
                        )
                    self.motion_manager.motion_ids[ref_env_ids] = matched_motion_ids
                    self.motion_manager.motion_times[ref_env_ids] = matched_motion_times
    ...
    def get_state_dict(self):
        ...
        """
        state_dict = {
            "control_manager": self.control_manager.get_state_dict(),
        }
    ...
    def load_state_dict(self, state_dict):
        ...
        """
        if self.motion_manager is not None and "motion_manager" in state_dict:
    ...
    def save_state(self) -> dict:
        ...
        snapshot = {
            ...
            # Evaluation shares the training environment. Preserve task state
            # as well as physics state so sampled commands, command timers,
            # curriculum EMAs, and reference tracking targets do not leak from
            # the evaluation rollout into the next PPO batch.
            "control_manager_state": _clone_snapshot_value(
                self.control_manager.get_state_dict()
            ),
        }
        if self.motion_manager is not None:
            # MotionManager.get_state_dict() intentionally stores persistent
            # sampling weights, not the per-environment runtime assignment.
            # Evaluation resets mutate these two buffers, so snapshot them
            # explicitly without undoing any deliberate weight update.
            snapshot["motion_manager_runtime"] = {
                "motion_ids": self.motion_manager.motion_ids.clone(),
                "motion_times": self.motion_manager.motion_times.clone(),
            }
        if self.state_history is not None:
    ...
    def restore_state(self, snapshot: dict) -> None:
        ...
        if "control_manager_state" in snapshot:
            self.control_manager.load_state_dict(snapshot["control_manager_state"])
        motion_runtime = snapshot.get("motion_manager_runtime")
        if self.motion_manager is not None and motion_runtime is not None:
            self.motion_manager.motion_ids.copy_(motion_runtime["motion_ids"])
            self.motion_manager.motion_times.copy_(motion_runtime["motion_times"])
        self._current_context = None
