...
class MotionManager:
    ...
    def __init__(
        ...
        # An explicit single-clip selection takes precedence over scene-motion
        # correspondence. This also lets MimicEvaluator discover that it should
        # evaluate and export only this clip.
        if self.config.fixed_motion_id is not None:
            fixed_motion_id = self.config.fixed_motion_id
            if type(fixed_motion_id) is not int:
                raise TypeError("fixed_motion_id must be an integer or None")
            if fixed_motion_id < 0 or fixed_motion_id >= self.motion_lib.num_motions():
                raise ValueError(
                    f"fixed_motion_id {fixed_motion_id} is out of range "
                    f"[0, {self.motion_lib.num_motions() - 1}]"
                )
            fixed_motion_ids_per_env = torch.full(
                (self.num_envs,),
                fixed_motion_id,
                dtype=torch.long,
                device=self.device,
        ...
        # Handle fixed motion IDs for scene-motion correspondence or explicit
