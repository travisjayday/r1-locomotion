...
class ControlComponent(ABC):
    ...
    def before_env_reset(self, env_ids: Tensor) -> None:
        """Prepare task state needed to choose a compatible reset state."""
        pass

    def sample_reference_reset(self, env_ids: Tensor, motion_manager):
        """Optionally return command-matched ``(motion_ids, motion_times)``."""
        return None

    def on_epoch_end(self, current_epoch: int) -> None:
        """Update training curricula after an epoch, if applicable."""
        pass
    ...
    def load_state_dict(self, state_dict: dict) -> None:
        """Restore checkpointable component state."""
        pass
