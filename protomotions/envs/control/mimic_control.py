# SPDX-FileCopyrightText: Copyright (c) 2025-2026 NVIDIA CORPORATION & AFFILIATES. All rights reserved.
# SPDX-License-Identifier: Apache-2.0

"""Mimic control component for motion tracking tasks.

This component manages reference motion tracking, including:
- Motion manager for motion library sampling and playback
- Reference state computation and terrain correction
- Masked mimic conditioning state
- Visualization markers for target poses
"""

from dataclasses import dataclass
from typing import Dict, List, Tuple, Union, TYPE_CHECKING

import torch
from torch import Tensor

from protomotions.envs.context_views import EnvContext, MimicContext
from protomotions.envs.control.base import ControlComponent, ControlComponentConfig
from protomotions.envs.obs.humanoid import dof_to_local
from protomotions.utils.rotations import quat_from_euler_xyz, quat_mul_norm
from protomotions.simulator.base_simulator.config import (
    MarkerConfig,
    VisualizationMarkerConfig,
    MarkerState,
)

if TYPE_CHECKING:
    from protomotions.envs.base_env.env import BaseEnv


@dataclass
class ReferenceMotionPerturbationConfig:
    """Uniform training noise applied only to policy reference commands."""

    joint_pos: float = 0.0
    joint_vel: float = 0.0
    root_pos_xyz: Tuple[float, float, float] = (0.0, 0.0, 0.0)
    root_ori_rpy: Tuple[float, float, float] = (0.0, 0.0, 0.0)


def perturb_reference_motion_commands(
    root_pos: Tensor,
    root_rot: Tensor,
    dof_pos: Tensor,
    dof_vel: Tensor,
    config: ReferenceMotionPerturbationConfig,
) -> tuple[Tensor, Tensor, Tensor, Tensor]:
    """Apply independent zero-mean uniform noise to a reference command."""

    command_root_pos = root_pos + (2.0 * torch.rand_like(root_pos) - 1.0) * torch.as_tensor(
        config.root_pos_xyz, device=root_pos.device, dtype=root_pos.dtype
    )
    command_dof_pos = dof_pos + (2.0 * torch.rand_like(dof_pos) - 1.0) * config.joint_pos
    command_dof_vel = dof_vel + (2.0 * torch.rand_like(dof_vel) - 1.0) * config.joint_vel

    rpy_limit = torch.as_tensor(
        config.root_ori_rpy, device=root_rot.device, dtype=root_rot.dtype
    )
    delta_rpy = (
        2.0
        * torch.rand(
            (*root_rot.shape[:-1], 3),
            device=root_rot.device,
            dtype=root_rot.dtype,
        )
        - 1.0
    ) * rpy_limit
    delta_rot = quat_from_euler_xyz(
        delta_rpy[..., 0], delta_rpy[..., 1], delta_rpy[..., 2], True
    )
    # Left multiplication interprets the requested roll/pitch/yaw offsets in
    # the world/root command frame and preserves unit quaternion length.
    command_root_rot = quat_mul_norm(delta_rot, root_rot, True)
    return command_root_pos, command_root_rot, command_dof_pos, command_dof_vel


@dataclass
class MimicControlConfig(ControlComponentConfig):
    """Configuration for mimic control component.

    Attributes:
        bootstrap_on_episode_end: If True, don't terminate when motion clip ends.
        future_steps: Future reference poses to provide in context. If int N,
            provides N consecutive steps (1 to N). If list, provides specific step
            indices (e.g., [1, 3, 5, 9, 15] for non-uniform sampling).
        reserve_future_context: End each tracked episode early enough that every
            configured future step can still be read from the original motion.
            The reserved tail remains in the motion library as observation context
            but is not sampled or exported as part of the physical rollout.
    """

    _target_: str = "protomotions.envs.control.mimic_control.MimicControl"

    bootstrap_on_episode_end: bool = True
    future_steps: Union[int, List[int]] = 1
    reserve_future_context: bool = False
    reference_motion_perturbation: ReferenceMotionPerturbationConfig | None = None


class MimicControl(ControlComponent):
    """Control component for motion tracking tasks.

    Provides context for mimic observations and rewards. Accesses the env's
    motion_manager rather than creating its own.
    """

    config: MimicControlConfig

    def __init__(self, config: MimicControlConfig, env: "BaseEnv"):
        """Initialize mimic control component.

        Args:
            config: Component configuration.
            env: Parent environment instance.
        """
        super().__init__(config, env)
        self._future_time_offsets_cache_key = None
        self._future_time_offsets_cache = None
        self._reference_motion_perturbations_enabled = (
            self.config.reference_motion_perturbation is not None
        )

        if self.config.reserve_future_context:
            future_steps = self.config.future_steps
            if isinstance(future_steps, int):
                step_indices = (future_steps,)
            else:
                step_indices = tuple(future_steps)
            if any(step < 0 for step in step_indices):
                raise ValueError("future_steps cannot contain negative values")
            max_future_step = max(step_indices, default=0)
            reserve_tail = getattr(
                self.env.motion_manager, "reserve_future_context_steps", None
            )
            if reserve_tail is None:
                raise TypeError(
                    "reserve_future_context requires a motion manager that supports "
                    "reserve_future_context_steps()"
                )
            reserve_tail(max_future_step)

    @property
    def reference_motion_perturbations_enabled(self) -> bool:
        return self._reference_motion_perturbations_enabled

    def set_reference_motion_perturbations_enabled(self, enabled: bool) -> None:
        """Enable/disable command noise, invalidating any cached context."""
        self._reference_motion_perturbations_enabled = bool(enabled)
        if hasattr(self.env, "_current_context"):
            self.env._current_context = None

    def step(self):
        """Control component step - motion manager is handled by env."""
        # Motion manager is updated by env.post_physics_step if needed
        pass

    def check_resets_and_terminations(self) -> Tuple[Tensor, Tensor]:
        """Check if motion clips have finished.

        Returns:
            Tuple of (reset_buf, terminate_buf) boolean tensors.
        """
        device = self.env.device
        num_envs = self.env.num_envs

        # Check if motion clip has finished (access via env)
        done_clip = self.env.motion_manager.get_done_tracks()
        reset_buf = done_clip

        # Only terminate if not bootstrapping
        if self.config.bootstrap_on_episode_end:
            terminate_buf = torch.zeros(num_envs, dtype=torch.bool, device=device)
        else:
            terminate_buf = done_clip

        return reset_buf, terminate_buf

    def _future_time_offsets(self) -> Tensor:
        future_steps = self.config.future_steps
        if isinstance(future_steps, int):
            step_indices = tuple(range(1, future_steps + 1))
        else:
            step_indices = tuple(future_steps)

        key = (step_indices, float(self.env.dt), torch.device(self.env.device))
        if key != self._future_time_offsets_cache_key:
            self._future_time_offsets_cache_key = key
            self._future_time_offsets_cache = self.env.dt * torch.as_tensor(
                step_indices, device=self.env.device, dtype=torch.float32
            )
        return self._future_time_offsets_cache

    def populate_context(self, ctx: EnvContext) -> None:
        """Populate mimic-specific view in the EnvContext.

        Creates a MimicContext with:
        - ref_state: Single-step reference state at current time (for rewards)
        - future_*: Multi-step future reference poses [envs, future_steps, ...]

        Args:
            ctx: The EnvContext to populate with ctx.mimic.
        """
        env_ids = getattr(ctx, "env_ids", None)
        if env_ids is None:
            motion_ids = self.env.motion_manager.motion_ids
            motion_times = self.env.motion_manager.motion_times
        else:
            motion_ids = self.env.motion_manager.motion_ids[env_ids]
            motion_times = self.env.motion_manager.motion_times[env_ids]
        num_envs = motion_ids.shape[0]

        # Build multi-step reference for observations
        time_offsets = self._future_time_offsets()
        future_steps = time_offsets.shape[0]
        future_times = motion_times.unsqueeze(-1) + time_offsets  # [envs, N]

        motion_lengths = self.env.motion_lib.get_motion_length(motion_ids)
        future_times = torch.minimum(future_times, motion_lengths.unsqueeze(-1))

        flat_motion_ids = (
            motion_ids.unsqueeze(-1).expand(num_envs, future_steps).reshape(-1)
        )
        flat_future_times = future_times.reshape(-1)

        # Query current and future states together to avoid two motion-lib passes.
        combined_motion_ids = torch.cat((motion_ids, flat_motion_ids), dim=0)
        combined_motion_times = torch.cat((motion_times, flat_future_times), dim=0)
        combined_state = self.env.motion_lib.get_motion_state(
            combined_motion_ids, combined_motion_times
        )
        ref_state = combined_state[:num_envs]
        future_state = combined_state[num_envs:]

        # Apply terrain height correction to reference state
        ref_gt = ref_state.rigid_body_pos.clone()
        if env_ids is None:
            ref_gt += self.env.get_spawn_to_ref_pose_offset_with_terrain_height_correction(
                ref_gt
            )
        else:
            ref_gt += self.env.get_spawn_to_ref_pose_offset_with_terrain_height_correction(
                ref_gt, env_ids=env_ids
            )
        ref_state.rigid_body_pos = ref_gt

        # Reshape to [envs, future_steps, ...] and apply terrain correction
        num_bodies = future_state.rigid_body_pos.shape[1]
        num_dofs = future_state.dof_pos.shape[1]

        # Body positions with terrain correction
        future_pos = future_state.rigid_body_pos.view(
            num_envs, future_steps, num_bodies, 3
        ).clone()
        if env_ids is None:
            offset = self.env.get_spawn_to_ref_pose_offset_with_terrain_height_correction(
                future_pos[:, 0, :, :]  # Use first step for offset
            )
        else:
            offset = self.env.get_spawn_to_ref_pose_offset_with_terrain_height_correction(
                future_pos[:, 0, :, :], env_ids=env_ids
            )
        future_pos += offset.unsqueeze(1)

        # Body rotations
        future_rot = future_state.rigid_body_rot.view(
            num_envs, future_steps, num_bodies, 4
        )

        # Body velocities
        future_vel = future_state.rigid_body_vel.view(
            num_envs, future_steps, num_bodies, 3
        )

        # Body angular velocities
        future_ang_vel = future_state.rigid_body_ang_vel.view(
            num_envs, future_steps, num_bodies, 3
        )

        # DOF positions and velocities
        future_dof_pos = future_state.dof_pos.view(num_envs, future_steps, num_dofs)
        future_dof_vel = future_state.dof_vel.view(num_envs, future_steps, num_dofs)

        hinge_axes_map = self.env.robot_config.kinematic_info.hinge_axes_map
        ref_lr = dof_to_local(ref_state.dof_pos, hinge_axes_map, True)

        command_root_pos = future_pos[:, :, self.env.robot_config.anchor_body_index, :]
        command_root_rot = future_rot[:, :, self.env.robot_config.anchor_body_index, :]
        command_dof_pos = future_dof_pos
        command_dof_vel = future_dof_vel
        if (
            self._reference_motion_perturbations_enabled
            and self.config.reference_motion_perturbation is not None
        ):
            (
                command_root_pos,
                command_root_rot,
                command_dof_pos,
                command_dof_vel,
            ) = perturb_reference_motion_commands(
                command_root_pos,
                command_root_rot,
                command_dof_pos,
                command_dof_vel,
                self.config.reference_motion_perturbation,
            )

        # Populate the mimic view
        ctx.mimic = MimicContext(
            ref_state=ref_state,
            future_pos=future_pos,
            future_rot=future_rot,
            future_vel=future_vel,
            future_ang_vel=future_ang_vel,
            future_dof_pos=future_dof_pos,
            future_dof_vel=future_dof_vel,
            anchor_idx=self.env.robot_config.anchor_body_index,
            ref_lr=ref_lr,
            command_root_pos=command_root_pos,
            command_root_rot=command_root_rot,
            command_dof_pos=command_dof_pos,
            command_dof_vel=command_dof_vel,
        )

    def create_visualization_markers(
        self, headless: bool
    ) -> Dict[str, VisualizationMarkerConfig]:
        """Create visualization markers for reference poses.

        Args:
            headless: If True, returns empty dict.

        Returns:
            Dictionary of marker configurations.
        """
        if headless:
            return {}

        visualization_markers = {}

        # Standard mimic: visualize all bodies
        body_names = self.env.robot_config.kinematic_info.body_names

        body_markers = []
        for body_name in body_names:
            if (
                self.env.robot_config.mimic_small_marker_bodies is not None
                and body_name in self.env.robot_config.mimic_small_marker_bodies
            ):
                body_markers.append(MarkerConfig(size="small"))
            else:
                body_markers.append(MarkerConfig(size="regular"))

        # Red markers for target poses
        body_markers_red_cfg = VisualizationMarkerConfig(
            type="sphere", color=(1.0, 0.0, 0.0), markers=body_markers
        )
        visualization_markers["body_markers_red"] = body_markers_red_cfg

        return visualization_markers

    def get_markers_state(self) -> Dict[str, MarkerState]:
        """Compute marker positions for reference poses.

        Returns:
            Dictionary mapping marker names to MarkerState.
        """
        if self.env.simulator.headless:
            return {}

        markers_state = {}

        # Get reference state at current time (access motion_manager via env)
        ref_state = self.env.motion_lib.get_motion_state(
            self.env.motion_manager.motion_ids, self.env.motion_manager.motion_times
        )

        target_pos = ref_state.rigid_body_pos.clone()
        target_pos += (
            self.env.get_spawn_to_ref_pose_offset_with_terrain_height_correction(
                target_pos
            )
        )

        # Standard mimic: show all body markers in red
        target_pos = target_pos.view(self.env.num_envs, -1, 3)
        markers_state["body_markers_red"] = MarkerState(
            translation=target_pos,
            orientation=torch.zeros(
                self.env.num_envs, target_pos.shape[1], 4, device=self.env.device
            ),
        )

        return markers_state
