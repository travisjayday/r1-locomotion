...
def compute_dof_pos_tracking_rew(
    dof_pos: Tensor,
    ref_dof_pos: Tensor,
    coefficient: float = -10.0,
) -> Tensor:
    """DeepMimic-style joint-angle tracking reward (exponential MSE).
    ...
    trajectory, which can leave tracking-precision unaddressed. This gives a
    small, explicit imitation signal toward a reference pose instead.

    Pairs with RandomYawRateSteeringCommandSourceConfig.track_reference_pose
    and dof_pos_tracking_rew_factory: ref_dof_pos is the pose of a reference
    clip matched to the currently active command and advanced in sync with
    elapsed real time since that command was set. When tracking is disabled
    for the active curriculum stage, ref_dof_pos is set to the agent's own
    current pose, making this term a no-op (reward saturates at its maximum)
    so the same reward component can stay wired across the whole curriculum
    without being added/removed per stage.

    Args:
        dof_pos: Current joint positions [num_envs, num_dofs].
        ref_dof_pos: Reference joint positions [num_envs, num_dofs].
        coefficient: Exponential coefficient for error (typically negative).

    Returns:
        Reward tensor [num_envs].
    """
    return mean_squared_error_exp(dof_pos, ref_dof_pos, coefficient)
...
def compute_contact_body_ori_rew(
    current_rigid_body_rot: Tensor,
    ref_rigid_body_rot: Tensor,
    ref_contacts: Tensor,
    body_indices: Tensor,
    ...
    """Track selected body orientations only while reference contact is active.

    This is intended for feet: an edge contact must not receive the same score
    as a flat reference sole merely because both produce a binary contact.  A
    sample with no selected reference contacts returns one, making the term a
    constant (and therefore gradient-free) during flight.
    """
    current = current_rigid_body_rot[:, body_indices]
    reference = ref_rigid_body_rot[:, body_indices]
    contact = ref_contacts[:, body_indices].to(current.dtype)
    angle_sq = quat_angle_diff_norm(current, reference, w_last=True)
    count = contact.sum(dim=-1)
    mean_contact_error = (angle_sq * contact).sum(dim=-1) / count.clamp_min(1.0)
    reward = torch.exp(-mean_contact_error / (sigma**2))
    return torch.where(count > 0.0, reward, torch.ones_like(reward))
