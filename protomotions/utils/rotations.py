...
def enforce_quaternion_hemisphere_continuity(quat: Tensor) -> Tensor:
    """Pick a consistent sign for a quaternion trajectory across time.

    q and -q represent the identical rotation, but a per-frame-independent
    conversion (e.g. a batched/vmapped rotation-matrix-to-quaternion call,
    where every frame is converted with no knowledge of its neighbors) can
    pick either sign arbitrarily frame to frame. Most consumers of a
    quaternion sequence (angle-based error metrics that wrap through
    atan2, hemisphere-aware slerp, angular velocity computed from rotation
    matrices instead of raw quaternion components) are already immune to
    this, but naive per-frame rendering or any consumer that does direct
    linear interpolation of raw quaternion components is not: it sees an
    apparent ~360-degree snap at the sign flip. This walks the sequence and
    flips whichever frames are needed so consecutive frames are always in
    the same hemisphere (dot product >= 0), without changing the rotation
    any frame represents.

    Args:
        quat: [T, ..., 4] quaternion trajectory (this codebase's convention:
            time first, e.g. rigid_body_rot is [T, num_bodies, 4]).
            Continuity is enforced along dim=0 independently for every other
            dimension (e.g. a body axis), regardless of xyzw/wxyz convention
            -- the fix only depends on the sign of the dot product between
            whole quaternions, which is convention-independent.

    Returns:
        Same shape, values identical up to a per-frame +-1 factor.
    """
    if quat.shape[0] < 2:
        return quat
    dots = (quat[1:] * quat[:-1]).sum(dim=-1)
    signs = torch.where(dots < 0, -torch.ones_like(dots), torch.ones_like(dots))
    ones = torch.ones_like(signs[:1])
    cumulative_sign = torch.cumprod(torch.cat([ones, signs], dim=0), dim=0)
    return quat * cumulative_sign.unsqueeze(-1)
