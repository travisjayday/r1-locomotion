...
class BaseAgent:
    ...
    def load(
        ...
        if checkpoint is not None:
            ...
            # Physical randomization stages are derived from the restored
            # absolute epoch rather than stored property tensors. Synchronize
            # before the first resumed rollout so it does not briefly train
            # with the epoch-0 envelope.
            if load_training_state:
                simulator = getattr(self.env, "simulator", None)
                simulator_epoch_end = getattr(simulator, "on_epoch_end", None)
                if simulator_epoch_end is not None:
                    simulator_epoch_end(self.current_epoch)
    ...
    def fit(self):
        ...
        while self.current_epoch < self.max_epochs:
            ...
            one_off_eval_due = (
                ...
                in getattr(self.evaluator.config, "eval_metrics_at_epochs", [])
            )
            checkpoint_eval_due = self.just_loaded_checkpoint_should_evaluate
            should_evaluate = self.evaluator is not None and (
                scheduled_eval_due or one_off_eval_due or checkpoint_eval_due
            )
            if should_evaluate:
