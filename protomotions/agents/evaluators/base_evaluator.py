...
class BaseEvaluator:
    ...
    def evaluate(self) -> Tuple[Dict, Optional[float], int]:
        ...
        # Evaluation runs in the same vectorized environment as training. It
        # must therefore be a transaction: resetting and stepping here may not
        # alter the physics, observation history, steering commands, reference
        # assignments, or curriculum statistics used by the next PPO rollout.
        env_snapshot = self.env.save_state()
        initialized = False
        ...
        try:
            ...
            self.run_evaluation()
            result = self.process_eval_results()
            self.eval_count += 1
            return result
        finally:
            try:
                ...
                self.env.restore_state(env_snapshot)
    ...
    def _policy_action(self, obs_td) -> Tensor:
        """Run only the actor when available and select its deterministic action.

        Evaluation never consumes task-value or discriminator outputs. Calling
        the full AMP model here needlessly executes three additional networks,
        which is particularly expensive for interactive single-environment GPU
        inference where kernel-launch overhead dominates.
        """
        actor = getattr(self.agent, "actor", None)
        model_outs = actor(obs_td) if actor is not None else self.agent.model(obs_td)
        return (
    ...
    def _disable_perturbations(self) -> None:
        ...
        # Reference-command perturbations improve the training policy's
        # robustness but must not contaminate clean evaluation, checkpoint
        # selection, video rollouts, or exported physical trajectories.
        self._saved_reference_motion_perturbations = []
        control_manager = getattr(self.env, "control_manager", None)
        for component in getattr(control_manager, "components", {}).values():
            if hasattr(component, "reference_motion_perturbations_enabled"):
                enabled = component.reference_motion_perturbations_enabled
                self._saved_reference_motion_perturbations.append(
                    (component, enabled)
                )
                component.set_reference_motion_perturbations_enabled(False)
    ...
    def _restore_perturbations(self) -> None:
        ...
        for component, enabled in getattr(
            self, "_saved_reference_motion_perturbations", []
        ):
            component.set_reference_motion_perturbations_enabled(enabled)
    ...
    def simple_test_policy(
        ...
        loop_rate = step / elapsed if elapsed > 0.0 else 0.0
        print(
            f"\nStopped after {step} steps. "
            f"Loop rate: {loop_rate:.1f} policy steps/s."
        )
