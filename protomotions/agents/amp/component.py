...
class AMPTrainingComponent:
    ...
    def augment_dataset(self, dataset):
        ...
        command_matching = getattr(
            self.config.amp_parameters,
            "command_matching",
            None,
        )
        if command_matching is not None:
            match_key = command_matching.observation_key
            if match_key not in discriminator_keys:
                raise ValueError(
                    f"AMP command matching key {match_key!r} must be a "
                    "discriminator input"
                )
            target_commands = dataset[match_key][:num_expert]
        expert_disc_obs = self.agent.get_expert_disc_obs(
            num_expert,
    ...
    def get_expert_disc_obs(
        self,
        num_samples: int,
        target_commands: Optional[Tensor] = None,
    ):
        command_matching = getattr(
            self.config.amp_parameters,
            "command_matching",
        ...
        if target_commands is not None:
            if command_matching is None:
                raise ValueError(
                    "target_commands require amp_parameters.command_matching"
                )
            motion_ids, motion_times, matched_commands = (
                sample_command_matched_motions(
                    self.agent.motion_manager,
                    target_commands=target_commands,
                    anchor_body_index=self.agent.env.robot_config.anchor_body_index,
                    candidate_count=command_matching.candidate_count,
                    command_scales=command_matching.command_scales,
                    smoothing_window_s=command_matching.smoothing_window_s,
                )
            )
            self.last_expert_command_match_mae = torch.mean(
        ...
        for start in range(0, num_samples, batch_size):
            ...
            if target_commands is not None:
                # Give expert and agent samples the exact same condition. The
                # selected reference frame supplies the motion, while sharing
                # the desired command prevents a discriminator shortcut based
                # only on differing command distributions.
                match_key = command_matching.observation_key
                chunk_obs[match_key] = target_commands[start:end]
    ...
    def _apply_command_dropout(self, obs: Dict[str, Tensor]) -> Dict[str, Tensor]:
        """Zero the discriminator's command condition for a random subset of rows.

        Without this, a conditional discriminator can learn to separate real
        from fake purely on whether the command "looks right" for the motion
        (expert transitions always carry their true, self-consistent command;
        early-training agent transitions often don't), which trivializes its
        classification task and leaves the style reward carrying no useful
        gradient toward matching motion *style* under a command. Dropping the
        command on a fraction of rows -- symmetrically for expert, agent, and
        replay batches -- forces the discriminator to keep using the motion
        features too.
        """
        command_matching = getattr(
            self.config.amp_parameters, "command_matching", None
        )
        if command_matching is None:
            return obs
        dropout_prob = getattr(command_matching, "command_dropout_prob", 0.0)
        if dropout_prob <= 0.0:
            return obs
        key = command_matching.observation_key
        if key not in obs:
            return obs
        command = obs[key]
        drop_mask = torch.rand(command.shape[0], device=command.device) < dropout_prob
        if not drop_mask.any():
            return obs
        obs = dict(obs)
        obs[key] = torch.where(drop_mask.unsqueeze(-1), torch.zeros_like(command), command)
        return obs
    ...
    def discriminator_step(
        ...
        *,
        ...
        agent_obs = self._apply_command_dropout(agent_obs)
        ...
        replay_obs = self._apply_command_dropout(replay_obs)
        ...
        expert_obs = self._apply_command_dropout(expert_obs)
        for val in expert_obs.values():
    ...
    def add_epoch_logging(self, training_log_dict) -> None:
        ...
        if hasattr(self, "last_expert_command_match_mae"):
            match_mae = self.last_expert_command_match_mae
            labels = ("vx", "vy", "yaw_rate")
            for index, label in enumerate(labels):
                training_log_dict[
                    f"amp/expert_command_match_{label}_mae"
                ] = match_mae[index]
...
class AMPAgentMixin:
    ...
    def get_expert_disc_obs(
        ...
        return AMPTrainingComponent.for_agent(self).get_expert_disc_obs(
            num_samples,
            target_commands=target_commands,
        )
