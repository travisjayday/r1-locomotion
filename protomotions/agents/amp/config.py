...
@dataclass
class AMPCommandMatchingConfig:
    """Match expert transitions to the policy's discriminator condition."""

    observation_key: str = "steering"
    candidate_count: int = 64
    command_scales: Tuple[float, float, float] = (0.5, 0.5, 0.5)
    smoothing_window_s: float = 0.2
    command_dropout_prob: float = field(
        default=0.0,
        metadata={
            "help": (
                "Probability of zeroing the command fed to the discriminator "
                "for a given transition, applied independently to expert, "
                "agent, and replay batches during discriminator_step. Without "
                "this, a conditional discriminator can classify real-vs-fake "
                "purely on whether the command looks self-consistent (real "
                "transitions always carry their true command; fake ones may "
                "not yet), which trivializes its task and stops the style "
                "reward from carrying useful gradient."
            ),
            "min": 0.0,
            "max": 1.0,
        },
...
class AMPParametersConfig:
    ...
    command_matching: Optional[AMPCommandMatchingConfig] = field(
        default=None,
        metadata={
            "help": (
                "Optional command-conditioned expert sampling. Expert motion "
                "frames are matched to agent commands and both discriminator "
                "classes receive the identical conditioning value."
            )
        },
    )
