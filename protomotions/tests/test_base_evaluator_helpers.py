...
class _Env:
    def __init__(self):
        ...
        self.reset_calls = []
        self.step_actions = []
        self.runtime_state = torch.tensor([3.0])
        self.restore_calls = 0

    def save_state(self):
        return {"runtime_state": self.runtime_state.clone()}

    def restore_state(self, snapshot):
        self.runtime_state.copy_(snapshot["runtime_state"])
...
def test_base_evaluator_evaluate_full_cycle_restores_and_logs_component_metrics(
    ...
    class _FullCycleEvaluator(BaseEvaluator):
        ...
        def run_evaluation(self):
            self.env.runtime_state.fill_(99.0)
    ...
    assert env.robot_config.reset_noise == "noise"
    assert env.simulator._push_enabled is True
    assert torch.equal(env.runtime_state, torch.tensor([3.0]))
    assert env.restore_calls == 1
...
def test_policy_action_uses_actor_without_running_full_model(tmp_path):
    env = _Env()
    agent = _Agent(env, tmp_path)
    evaluator = BaseEvaluator(agent, _Fabric(), _config())
    mean_action = torch.ones(2, 1)

    agent.actor = lambda obs_td: {"mean_action": mean_action}
    agent.model = lambda obs_td: (_ for _ in ()).throw(
        AssertionError("full model should not run during evaluation")
    )
