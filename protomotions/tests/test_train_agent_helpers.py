...
def test_apply_resume_training_iteration_limit(monkeypatch, tmp_path):
    module = _load_train_agent_globals(monkeypatch, tmp_path)
    apply_limit = module["apply_resume_training_iteration_limit"]
    config = SimpleNamespace(training_max_iterations=None)

    apply_limit(2502, config)
    assert config.training_max_iterations == 2502

    apply_limit(None, config)
    assert config.training_max_iterations == 2502
...
def test_main_resume_isaaclab_uses_saved_configs_launcher_without_skip_flag(
    monkeypatch,
    tmp_path,
):
    module = _load_train_agent_globals(monkeypatch, tmp_path)
    ...
    assert ("agent_fit", False) in calls
    assert (
        "app_launcher",
        {
            "headless": False,
