...
def test_inference_parser_requires_checkpoint_and_parses_options(monkeypatch, tmp_path):
    ...
    parsed = parser.parse_args(
        [
            ...
            "0.5",
            "--kit-args=--/rtx/pathtracing/cached/retrace=0.1 --/rtx/pathtracing/fractionalCutoutOpacity=false",
            "--command-source",
            "target=keyboard",
            "--overrides",
    ...
    assert parsed.viewport_resolution_scale == pytest.approx(0.5)
    assert parsed.kit_args == (
...
def test_inference_command_source_override_sets_steering_keyboard_source(
    monkeypatch,
    tmp_path,
):
    ...
    env_config = SimpleNamespace(
        control_components={
            "steering": SteeringControlConfig(
                command_source=RandomSteeringCommandSourceConfig()
            )
        }
    )

    module["apply_command_source_overrides"](
        env_config, ["steering=keyboard"]
    )

    command_source = env_config.control_components["steering"].command_source
    assert isinstance(command_source, KeyboardSteeringCommandSourceConfig)


def test_inference_command_source_override_sets_steering_window_source(
    monkeypatch,
    tmp_path,
):
    from protomotions.envs.control.steering_control import (
    ...
    module["apply_command_source_overrides"](
        env_config, ["steering=window"]
    )

    command_source = env_config.control_components["steering"].command_source
    assert isinstance(command_source, WindowSteeringCommandSourceConfig)


def test_inference_command_source_override_preserves_yaw_rate_sampling(
    monkeypatch,
    tmp_path,
):
    from protomotions.envs.control.steering_control import (
    ...
    module["apply_command_source_overrides"](
        env_config, ["steering=random"]
    )

    command_source = env_config.control_components["steering"].command_source
    assert isinstance(command_source, RandomYawRateSteeringCommandSourceConfig)


def test_interactive_inference_disables_unused_contact_sensors(
    monkeypatch,
    tmp_path,
):
    checkpoint = tmp_path / "last.ckpt"
    ...
    env_config = SimpleNamespace(
        observation_components={"proprio": _Component("current.dof_pos")},
        termination_components={
            "fallen": _Component("current.anchor_pos"),
            "contact_fall": _Component("current.rigid_body_contacts"),
        },
        scene_obs=SimpleNamespace(enabled=False),
    )
    robot_config = SimpleNamespace(contact_bodies=["all"])

    module["apply_interactive_inference_optimizations"](
        env_config, robot_config
    )

    assert robot_config.contact_bodies is None
    assert list(env_config.termination_components) == ["fallen"]


def test_interactive_inference_preserves_required_contact_sensors(
    monkeypatch,
    tmp_path,
):
    checkpoint = tmp_path / "last.ckpt"
    module = _load_inference_agent_globals(monkeypatch, checkpoint)
    contact_observation = SimpleNamespace(
        get_bindings_dict=lambda: {"contacts": "current.rigid_body_contacts"}
    )
    env_config = SimpleNamespace(
        observation_components={"proprio": contact_observation},
        termination_components={},
        scene_obs=SimpleNamespace(enabled=False),
    )
    robot_config = SimpleNamespace(contact_bodies=["feet"])

    module["apply_interactive_inference_optimizations"](
        env_config, robot_config
    )

    assert robot_config.contact_bodies == ["feet"]


def test_interactive_inference_ignores_disabled_optional_contact_binding(
    monkeypatch,
    tmp_path,
):
    checkpoint = tmp_path / "last.ckpt"
    ...
    observation = SimpleNamespace(
        get_bindings_dict=lambda: {"contacts": "historical.body_contacts"},
        get_params=lambda: {"observe_contacts": False},
    )
    env_config = SimpleNamespace(
        observation_components={"history": observation},
        termination_components={},
        scene_obs=SimpleNamespace(enabled=False),
    )
    robot_config = SimpleNamespace(contact_bodies=["all"])
...
def test_interactive_window_disables_duplicate_viewport_command_markers(
    monkeypatch,
    tmp_path,
):
    from protomotions.envs.control.steering_control import (
    ...
    robot_config = SimpleNamespace(contact_bodies=None)

    module["apply_interactive_inference_optimizations"](
        env_config, robot_config
    )

    assert env_config._disable_visualization_markers is True
