...
def test_mimic_process_eval_results_saves_predicted_motion_lib_on_interval(tmp_path):
    ...
    evaluator._save_predicted_motion_lib = (
        lambda metrics, epoch, sample_dt=None: saved.append((metrics, epoch))
    )
...
def test_mimic_motion_sampling_weights_cap_bounds_repeated_failures(tmp_path):
    """A persistently failing motion must not grow without bound.

    The failure branch divides by the discount every evaluation, so before the
    cap a clip that never succeeded compounded forever: r1_locomotion_10
    reached a weight of 991429 against a 0.05 floor, leaving an effective
    sample size of 39 of 908 motions and collapsing the training set onto the
    handful of clips the policy could not do.
    """
    evaluator = _evaluator(
        tmp_path,
        motion_weights_rules=MotionWeightsRulesConfig(
            motion_weights_update_success_discount=0.5,
            motion_weights_update_failure_discount=0.5,
            min_motion_weight=0.1,
            max_motion_weight_ratio=20.0,
        ),
    )
    # Motion 1 fails every single evaluation; the other two always succeed.
    evaluator._motion_failed = torch.tensor([False, True, False])
    for _ in range(40):
        evaluator._update_motion_sampling_weights()
        evaluator.env.motion_manager.motion_weights = (
            evaluator.env.motion_manager.updated_weights.clone()
        )

    weights = evaluator.env.motion_manager.updated_weights
    # floor 0.1, ratio 20 -> the hardest clip may be sampled at most 20x the
    # easiest, no matter how many times it fails.
    assert float(weights.max()) <= 0.1 * 20.0 + 1e-6
    assert float(weights.min()) >= 0.1 - 1e-6
    assert float(weights.max()) / float(weights.min()) <= 20.0 + 1e-6
    # The cap must not disable the curriculum: the failing clip is still the
    # most-sampled one by a wide margin.
    assert weights[1] > weights[0] * 10


def test_mimic_motion_sampling_weights_cap_of_one_disables_reweighting(tmp_path):
    """ratio=1 collapses the curriculum to uniform, a usable escape hatch."""
    evaluator = _evaluator(
        tmp_path,
        motion_weights_rules=MotionWeightsRulesConfig(
            motion_weights_update_success_discount=0.5,
            motion_weights_update_failure_discount=0.5,
            min_motion_weight=0.25,
            max_motion_weight_ratio=1.0,
        ),
    )
    evaluator._motion_failed = torch.tensor([False, True, False])
    for _ in range(5):
