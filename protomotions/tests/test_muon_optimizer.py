...
def _make_lr_test_agent(kl_ema_alpha, desired_kl=0.01):
    model = nn.Linear(2, 1)
    actor_optimizer = torch.optim.SGD(model.parameters(), lr=0.01)
    critic_optimizer = torch.optim.SGD(model.parameters(), lr=0.01)
    return SimpleNamespace(
        ...
        config=SimpleNamespace(
            adaptive_lr=SimpleNamespace(
                ...
                min_lr=1e-7,
                max_lr=1.0,
                kl_ema_alpha=kl_ema_alpha,
            )
        ),
    )


def test_adaptive_lr_kl_ema_alpha_one_matches_raw_kl_prior_behavior():
    agent = _make_lr_test_agent(kl_ema_alpha=1.0)
    # A single large reading immediately crosses 2x desired_kl and cuts LR,
    # exactly like the pre-EMA implementation (kl_ema_alpha didn't exist).
    PPO._update_learning_rate(agent, kl_mean=0.05)
    assert agent.actor_lr == pytest.approx(0.01 / 1.5)
    assert agent._kl_ema == pytest.approx(0.05)


def test_adaptive_lr_kl_ema_smooths_a_single_outlier_reading():
    agent = _make_lr_test_agent(kl_ema_alpha=0.15)
    # Seed the EMA at an on-target baseline, as it would be after many normal
    # epochs of real training (the first-ever call has no history to blend
    # with, so it seeds _kl_ema directly -- this call establishes that history
    # rather than testing the behavior under test).
    PPO._update_learning_rate(agent, kl_mean=0.01)
    assert agent._kl_ema == pytest.approx(0.01)
    assert agent.actor_lr == pytest.approx(0.01)  # at target: unchanged

    # Now a 5x-over-target spike (the post-eval-scale event), smoothed:
    # ema = 0.15*0.05 + 0.85*0.01 = 0.016, under the 0.02 (2x desired_kl)
    # threshold that would otherwise cut the LR on this one reading alone.
    PPO._update_learning_rate(agent, kl_mean=0.05)
    assert agent._kl_ema == pytest.approx(0.016)
    assert agent.actor_lr == pytest.approx(0.01)  # unchanged: no threshold crossed

    # A second consecutive high reading compounds the EMA until it does
    # eventually cross -- smoothing delays and dampens the reaction to a
    # sustained shift, it doesn't hide one that persists.
    PPO._update_learning_rate(agent, kl_mean=0.05)
    assert agent._kl_ema == pytest.approx(0.15 * 0.05 + 0.85 * 0.016)
    assert agent.actor_lr == pytest.approx(0.01 / 1.5)
