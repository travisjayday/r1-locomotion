...
def test_physx_material_setter_uses_cpu_environment_indices():
    simulator_path = (
        Path(__file__).parents[1] / "simulator" / "isaaclab" / "simulator.py"
    )
    tree = ast.parse(simulator_path.read_text())
    ...
    all_env_ids_assignment = next(
        node
        for node in apply_randomization.body
        if isinstance(node, ast.Assign)
        and any(
            isinstance(target, ast.Name) and target.id == "all_env_ids"
            for target in node.targets
        )
    )

    assert isinstance(all_env_ids_assignment.value, ast.Call)
    assert any(
        keyword.arg == "device"
        and isinstance(keyword.value, ast.Constant)
        and keyword.value.value == "cpu"
        for keyword in all_env_ids_assignment.value.keywords
    )
