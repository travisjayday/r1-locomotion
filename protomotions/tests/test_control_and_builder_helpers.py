...
def test_reference_motion_command_perturbations_match_omnitrack_ranges():
    torch.manual_seed(7)
    root_pos = torch.zeros(256, 4, 3)
    root_rot = _identity_quat(256, 4)
    dof_pos = torch.zeros(256, 4, 26)
    ...
    noisy_root_pos, noisy_root_rot, noisy_dof_pos, noisy_dof_vel = (
        perturb_reference_motion_commands(
            root_pos, root_rot, dof_pos, dof_vel, config
        )
    )

    assert torch.max(torch.abs(noisy_root_pos)) <= 0.01
    assert torch.max(torch.abs(noisy_dof_pos)) <= 0.01
    assert torch.max(torch.abs(noisy_dof_vel)) <= 0.5
    assert torch.any(noisy_root_pos != root_pos)
    assert torch.any(noisy_dof_pos != dof_pos)
    assert torch.any(noisy_dof_vel != dof_vel)
    assert torch.any(noisy_root_rot != root_rot)
    assert torch.allclose(
        torch.linalg.vector_norm(noisy_root_rot, dim=-1),
        torch.ones(noisy_root_rot.shape[:-1]),
        atol=1.0e-6,
    )
