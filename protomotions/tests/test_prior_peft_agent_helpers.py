...
def test_prior_peft_fit_runs_one_epoch_rollout_eval_and_checkpoint_branches(
    ...
    assert agent._skip_next_policy_update is False
...
def test_prior_peft_fit_ignores_legacy_skip_flag_and_can_stop_training(monkeypatch):
    calls = []
    buffer = _ExperienceBufferRecorder()
    monkeypatch.setattr(
        base_agent_module,
    ...
    agent.pre_process_dataset = lambda: calls.append(("unexpected_pre_process",))
    agent.optimize_model = lambda: calls.append(("optimize",)) or {"train/loss": 2.0}
    agent.save = lambda checkpoint_name, new_high_score=False: calls.append(
        ("save", checkpoint_name, new_high_score)
    )
    agent.post_epoch_logging = lambda log_dict: calls.append(("post_log", dict(log_dict)))
    ...
    assert buffer.make_dict_calls == 0
    assert ("unexpected_pre_process",) not in calls
    assert ("optimize",) in calls
    assert ("post_log", {"train/loss": 2.0, "epoch": 0}) in calls
