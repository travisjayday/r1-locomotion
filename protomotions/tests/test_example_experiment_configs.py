...
from examples.experiments.steering import r1_steering
...
class _R1SteeringRobotConfig:
    number_of_actions = 3
    anchor_body_index = 0

    def __init__(self):
        dof_names = ["left_hip_pitch_joint", "head_pitch_joint", "head_yaw_joint"]
        self.kinematic_info = SimpleNamespace(
            num_dofs=3,
            dof_names=dof_names,
            dof_limits_lower=torch.tensor([-2.0, -1.0, -1.0]),
            dof_limits_upper=torch.tensor([2.0, 1.0, 1.0]),
            body_names=[
                "pelvis_link",
        ...
        self.default_dof_pos = torch.zeros(3)
        self.reset_noise = object()
...
def test_r1_steering_uses_teacher_action_interface_and_safety_components():
    args = _args()
    robot_cfg = _R1SteeringRobotConfig()
    simulator_cfg = SimpleNamespace(
        sim=SimpleNamespace(fps=100, decimation=2),
        domain_randomization=object(),
    )

    r1_steering.configure_robot_and_simulator(robot_cfg, simulator_cfg, args)
    env_cfg = r1_steering.env_config(robot_cfg, args)

    assert simulator_cfg.sim.fps == 200
    assert simulator_cfg.sim.decimation == 4
    assert simulator_cfg.domain_randomization is not None
    dr_cfg = simulator_cfg.domain_randomization
    assert [stage.start_epoch for stage in dr_cfg.curriculum] == [0, 750, 1500]
    assert [stage.intensity for stage in dr_cfg.curriculum] == pytest.approx(
        [0.5, 0.75, 1.0]
    )
    assert dr_cfg.actuator.stiffness_scale_range == pytest.approx((0.85, 1.15))
    assert dr_cfg.actuator.damping_scale_range == pytest.approx((0.8, 1.2))
    assert dr_cfg.actuator.motor_strength_scale_range == pytest.approx((0.85, 1.15))
    assert dr_cfg.actuator.delay_steps_range == (0, 2)
    assert dr_cfg.rigid_body.mass_scale_range == pytest.approx((0.9, 1.1))
    assert dr_cfg.rigid_body.inertia_scale_range == pytest.approx((0.9, 1.1))
    mild_dr = dr_cfg.resolved_for_epoch(0)
    assert mild_dr.action_noise.action_noise_range == pytest.approx((-0.01, 0.01))
    assert mild_dr.observation_noise.dof_vel_noise == pytest.approx(0.2)
    assert mild_dr.actuator.delay_steps_range == (0, 1)
    assert "left_wrist_roll_link" in robot_cfg.contact_bodies
    assert "left_hip_yaw_link" in robot_cfg.contact_bodies
    assert env_cfg.action_config["fn"].__name__ == "bm_pd_action"
    assert torch.allclose(
        env_cfg.action_config["action_scale"], torch.full((3,), 2.0)
    )
    assert torch.equal(
        env_cfg.action_config["action_enabled"], torch.tensor([1.0, 0.0, 0.0])
    )
    assert set(env_cfg.reward_components) == {
        "track_lin_vel_xy",
        "track_yaw_rate",
        "lin_vel_z",
        "ang_vel_xy",
    ...
    assert set(env_cfg.termination_components) == {
        "fallen",
        "unwanted_ground_contact",
    }
    assert env_cfg.reward_components["track_lin_vel_xy"].static_params[
        "weight"
    ] == pytest.approx(2.0)
    steering_cfg = env_cfg.control_components["steering"]
    assert steering_cfg.tar_speed_max == pytest.approx(1.8)
    assert steering_cfg.yaw_rate_commands is True
    assert steering_cfg.command_source.turn_in_place_probability == pytest.approx(
        0.15
    )
    assert steering_cfg.command_source.high_speed_probability == pytest.approx(0.25)
    assert steering_cfg.command_source.low_speed_probability == pytest.approx(0.25)
    assert steering_cfg.command_source.low_speed_max == pytest.approx(0.8)
    curriculum = steering_cfg.command_source.curriculum
    assert [stage.start_epoch for stage in curriculum] == [0, 500, 1000, 1500]
    assert curriculum[0].tar_speed_min == pytest.approx(0.3)
    assert curriculum[0].tar_speed_max == pytest.approx(1.0)
    assert curriculum[0].stand_probability == pytest.approx(0.0)
    assert curriculum[0].moving_yaw_rate_probability == pytest.approx(0.0)
    assert curriculum[0].advance_lin_vel_reward_threshold == pytest.approx(0.60)
    assert curriculum[0].advance_yaw_rate_reward_threshold == pytest.approx(0.80)
    assert [stage.init_start_prob for stage in curriculum] == pytest.approx(
        [1.0, 0.75, 0.5, 0.5]
    )
    assert curriculum[-1].turn_in_place_probability == pytest.approx(0.15)
    assert curriculum[-1].backward_probability == pytest.approx(0.25)
    assert curriculum[-1].backward_speed_max == pytest.approx(0.7)
    assert curriculum[-1].high_speed_probability == pytest.approx(0.25)
    assert curriculum[-1].high_speed_min == pytest.approx(1.2)
    assert curriculum[-1].low_speed_probability == pytest.approx(0.25)
    assert curriculum[-1].low_speed_max == pytest.approx(0.8)
    assert curriculum[-1].moving_yaw_rate_probability == pytest.approx(0.50)
    assert curriculum[-1].yaw_rate_max_abs == pytest.approx(0.90)
    assert curriculum[-1].track_reference_pose is True
    assert [stage.minimum_epochs for stage in curriculum] == [500, 300, 400, 0]
    assert steering_cfg.command_source.match_reference_resets is True
    assert steering_cfg.command_source.reference_match_candidate_count == 512
    assert env_cfg.motion_manager.init_start_prob == pytest.approx(1.0)
    agent_cfg = r1_steering.agent_config(robot_cfg, env_cfg, args)
    assert agent_cfg.model.actor.actor_logstd == pytest.approx(-2.3)
    assert agent_cfg.symmetry.coef == pytest.approx(2.0)
    assert agent_cfg.task_reward_w == pytest.approx(0.5)
    assert agent_cfg.amp_parameters.discriminator_reward_w == pytest.approx(0.5)
    assert agent_cfg.amp_parameters.command_matching.observation_key == "steering"
    assert agent_cfg.amp_parameters.command_matching.candidate_count == 512
    assert "steering" in agent_cfg.model.discriminator.in_keys
    assert "steering" in agent_cfg.model.disc_critic.in_keys
    assert "steering" in agent_cfg.reference_obs_components
    assert agent_cfg.adaptive_lr.enabled is True
    assert agent_cfg.adaptive_lr.desired_kl == pytest.approx(0.008)
    assert agent_cfg.adaptive_lr.min_lr == pytest.approx(1.0e-6)
    assert agent_cfg.adaptive_lr.max_lr == pytest.approx(1.0e-4)
    assert agent_cfg.model.discriminator_optimizer.lr == pytest.approx(1.0e-5)
    assert agent_cfg.evaluator._target_.endswith("SteeringEvaluator")

    warm_start_env_cfg = r1_steering.env_config(
        robot_cfg, _args(checkpoint="results/r1_steering8/last.ckpt")
    )
    assert (
        warm_start_env_cfg.control_components["steering"]
        .command_source.curriculum
        is None
    )
