...
def test_explicit_actuator_models_group_identical_joint_configurations():
    shared = dict(
        stiffness=40.0,
        damping=2.0,
        armature=0.01,
        ...
        torque_speed_x1=10.0,
        torque_speed_x2=20.0,
        torque_same_direction=30.0,
        torque_opposite_direction=25.0,
        friction_static=0.4,
        friction_dynamic=0.2,
        friction_activation_velocity=0.1,
    )
    control_info = {
        "left.joint": ControlInfo(**shared),
        "right+joint": ControlInfo(**shared),
        "head": ControlInfo(stiffness=10.0, damping=1.0),
    }

    specs = resolve_actuator_specs_for_control_type(
        control_info, ControlType.BUILT_IN_PD
    )

    assert len(specs) == 2
    assert specs[0].joint_names_expr == (r"left\.joint", r"right\+joint")
    assert specs[0].actuator_model == "unitree"
    assert specs[1].joint_names_expr == ("head",)
    assert specs[1].actuator_model is None


def test_r1_explicit_actuator_groups_cover_each_joint_once():
    config = robot_config("r1")
    specs = resolve_actuator_specs_for_control_type(
        config.control.control_info, ControlType.BUILT_IN_PD
    )

    expressions = [expr for spec in specs for expr in spec.joint_names_expr]
    assert Counter(expressions) == Counter(
        re.escape(name) for name in config.control.control_info
    )
    assert len(specs) < len(config.control.control_info)
