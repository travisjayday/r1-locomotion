...
def test_steering_keyboard_source_updates_active_env_and_releases_bindings():
    simulator = _FakeSimulator(
        [[0.0, 0.0, 0.9], [2.0, 0.0, 0.9]], headless=False
    )
    env = SimpleNamespace(
        num_envs=2,
        device=torch.device("cpu"),
        progress_buf=torch.zeros(2, dtype=torch.long),
        reset_buf=torch.ones(2, dtype=torch.bool),
        terminate_buf=torch.zeros(2, dtype=torch.bool),
        simulator=simulator,
        robot_config=SimpleNamespace(anchor_body_index=0),
    )
    control = SteeringControl(
        SteeringControlConfig(
            tar_speed_max=2.0,
            command_source=KeyboardSteeringCommandSourceConfig(
                default_speed=1.25,
                fail_if_headless=False,
            ),
        ),
    ...
    simulator.user_interface.handle_key_event("W", pressed=True)
    control.step()
    assert control._tar_speed.tolist() == [0.0, 1.25]
    assert torch.allclose(control._tar_dir[1], torch.tensor([1.0, 0.0]))

    simulator.user_interface.handle_key_event("Q", pressed=True)
    control.step()
    assert control._tar_face_dir[1, 1] > 0.0

    simulator.user_interface.handle_key_event("SPACE", pressed=True)
    control.step()
    assert control._tar_speed[1] == 0.0

    control.close()
    assert "W" not in simulator.user_interface.registered_keys
    assert "Q" not in simulator.user_interface.registered_keys
    assert "E" not in simulator.user_interface.registered_keys
    assert "SPACE" not in simulator.user_interface.registered_keys


def test_steering_external_heading_relative_command_rotates_into_world_frame():
    half_sqrt = 2.0**-0.5
    simulator = _FakeSimulator(
        [[0.0, 0.0, 0.9]],
        headless=True,
    )
    simulator.robot_state.rigid_body_rot[0, 0] = torch.tensor(
        [0.0, 0.0, half_sqrt, half_sqrt]
    )
    env = SimpleNamespace(
        num_envs=1,
        device=torch.device("cpu"),
        progress_buf=torch.zeros(1, dtype=torch.long),
        reset_buf=torch.ones(1, dtype=torch.bool),
        terminate_buf=torch.zeros(1, dtype=torch.bool),
        simulator=simulator,
        robot_config=SimpleNamespace(anchor_body_index=0),
    )
    control = SteeringControl(SteeringControlConfig(tar_speed_max=2.0), env)

    control.set_heading_relative_command(
        torch.tensor([0]),
        movement_direction=torch.tensor([[1.0, 0.0]]),
        speed=torch.tensor([1.5]),
        facing_direction=torch.tensor([[1.0, 0.0]]),
    )

    assert torch.allclose(control._tar_dir[0], torch.tensor([0.0, 1.0]), atol=1e-5)
    assert torch.allclose(
        control._tar_face_dir[0], torch.tensor([0.0, 1.0]), atol=1e-5
    )
    assert control._tar_speed[0] == 1.5


def test_yaw_rate_steering_samples_turn_in_place_and_keyboard_commands():
    simulator = _FakeSimulator([[0.0, 0.0, 0.9]], headless=False)
    env = SimpleNamespace(
        num_envs=1,
        device=torch.device("cpu"),
    ...
    control = SteeringControl(
        SteeringControlConfig(
            ...
            command_source=RandomYawRateSteeringCommandSourceConfig(
                ...
                yaw_rate_min_abs=0.4,
                yaw_rate_max_abs=0.8,
            ),
        ),
        env,
    )
    control.reset(torch.tensor([0]))
    assert torch.equal(control._tar_local_vel, torch.zeros(1, 2))
    assert 0.4 <= torch.abs(control._tar_yaw_rate[0]) <= 0.8

    control.close()
    keyboard = SteeringControl(
        SteeringControlConfig(
            tar_speed_max=1.6,
            yaw_rate_commands=True,
            command_source=KeyboardSteeringCommandSourceConfig(
                default_speed=1.0,
                default_yaw_rate=0.7,
                fail_if_headless=False,
            ),
        ),
        env,
    )
    keyboard.reset(torch.tensor([0]))
    simulator.user_interface.handle_key_event("W", pressed=True)
    keyboard.step()
    assert torch.allclose(keyboard._tar_local_vel[0], torch.tensor([1.0, 0.0]))
    simulator.user_interface.handle_key_event("W", pressed=False)
    simulator.user_interface.handle_key_event("Q", pressed=True)
    keyboard.step()
    assert keyboard._tar_yaw_rate[0] == 0.7

    # Keyboard inference replaces the training-time random command source.
    # Keep reference-pose tracking rewards wired as a no-op rather than
    # passing None into their tensor kernels.
    dof_pos = torch.tensor([[0.1, -0.2]])
    ctx = SimpleNamespace(current=SimpleNamespace(dof_pos=dof_pos))
    keyboard.populate_context(ctx)
    assert ctx.steering.ref_dof_pos is dof_pos
    keyboard.close()


def test_yaw_rate_steering_can_force_jogging_speed_samples():
    num_envs = 2048
    simulator = _FakeSimulator([[0.0, 0.0, 0.9]] * num_envs, headless=True)
    env = SimpleNamespace(
        num_envs=num_envs,
        device=torch.device("cpu"),
        progress_buf=torch.zeros(num_envs, dtype=torch.long),
        reset_buf=torch.ones(num_envs, dtype=torch.bool),
        terminate_buf=torch.zeros(num_envs, dtype=torch.bool),
        simulator=simulator,
        robot_config=SimpleNamespace(anchor_body_index=0),
    )
    control = SteeringControl(
        SteeringControlConfig(
            ...
            command_source=RandomYawRateSteeringCommandSourceConfig(
                ...
                forward_heading_range=0.0,
                high_speed_probability=1.0,
                high_speed_min=1.2,
    ...
    assert torch.all(control._tar_speed >= 1.2)
    assert torch.all(control._tar_speed <= 1.8)
    assert torch.allclose(control._tar_local_vel[:, 1], torch.zeros(num_envs))


def test_yaw_rate_steering_can_force_low_speed_samples():
    num_envs = 2048
    simulator = _FakeSimulator([[0.0, 0.0, 0.9]] * num_envs, headless=True)
    env = SimpleNamespace(
        num_envs=num_envs,
    ...
    assert torch.all(control._tar_speed >= 0.2)
    assert torch.all(control._tar_speed <= 0.8)
    assert torch.allclose(control._tar_local_vel[:, 1], torch.zeros(num_envs))


def test_yaw_rate_steering_low_and_high_speed_probabilities_are_mutually_exclusive():
    num_envs = 4096
    simulator = _FakeSimulator([[0.0, 0.0, 0.9]] * num_envs, headless=True)
    env = SimpleNamespace(
        num_envs=num_envs,
    ...
    control = SteeringControl(
        SteeringControlConfig(
            ...
            command_source=RandomYawRateSteeringCommandSourceConfig(
                ...
                omnidirectional_probability=0.0,
                forward_heading_range=0.0,
                low_speed_probability=0.3,
                low_speed_max=0.8,
                high_speed_probability=0.3,
                high_speed_min=1.2,
                moving_yaw_rate_probability=0.0,
            ),
        ),
    ...
    is_low = control._tar_speed <= 0.8
    is_high = control._tar_speed >= 1.2
    is_mid = ~is_low & ~is_high
    # Every sample lands in exactly one band -- no low/high overlap.
    assert torch.all(is_low ^ is_high ^ is_mid)
    # 30% forced into each override band, plus a share of the remaining 40%
    # baseline uniform(0.2, 1.8) draw that happens to land in the same band:
    # P(low) = P(high) = 0.3 + 0.4 * (0.6 / 1.6) = 0.45; P(mid) = 0.10.
    assert 0.40 < is_low.float().mean() < 0.50
    assert 0.40 < is_high.float().mean() < 0.50
    assert 0.05 < is_mid.float().mean() < 0.15


def test_window_steering_source_uses_standalone_actions_and_reports_status(
    monkeypatch,
):
    import protomotions.envs.control.steering_control as steering_module

    class _FakeCommandWindow:
        instances = []

        def __init__(self, **kwargs):
            self.kwargs = kwargs
            self.actions = ["move_forward", "face_left", "faster"]
            self.statuses = []
            self.closed = False
    ...
    monkeypatch.setattr(
        steering_module, "SteeringCommandWindow", _FakeCommandWindow
    )
    simulator = _FakeSimulator([[0.0, 0.0, 0.9]], headless=False)
    env = SimpleNamespace(
        num_envs=1,
    ...
    control = SteeringControl(
        SteeringControlConfig(
            tar_speed_max=1.6,
            yaw_rate_commands=True,
            command_source=WindowSteeringCommandSourceConfig(
                default_speed=1.0,
                default_yaw_rate=0.7,
    ...
    window = _FakeCommandWindow.instances[0]
    assert "W" not in simulator.user_interface.registered_keys
    assert torch.allclose(control._tar_local_vel[0], torch.tensor([1.25, 0.0]))
    assert control._tar_yaw_rate[0] == pytest.approx(0.7)
    assert window.statuses[-1]["local_vx"] == pytest.approx(1.25)
    assert window.statuses[-1]["yaw_rate"] == pytest.approx(0.7)

    control.close()
    assert window.closed is True


def test_steering_command_window_maps_keys_and_formats_status():
    assert SteeringCommandWindow.action_for_keysym("w") == "move_forward"
    assert SteeringCommandWindow.action_for_keysym("SPACE") == "stop"
    assert SteeringCommandWindow.action_for_keysym("plus") == "faster"
    assert SteeringCommandWindow.action_for_keysym("F1") is None

    text = SteeringCommandWindow.format_status(
        {
            "env_id": 2,
            "mode": "body velocity + yaw rate",
            "local_vx": 1.25,
            "local_vy": -0.5,
            "speed": 1.35,
            "yaw_rate": 0.7,
            "move_heading_deg": 45.0,
            "face_heading_deg": 60.0,
        }
    )
    assert "Environment: 2" in text
    assert "local vx: +1.25 m/s" in text
    assert "yaw rate: +0.70 rad/s" in text


def test_yaw_rate_steering_curriculum_advances_and_restores_stage():
    simulator = _FakeSimulator([[0.0, 0.0, 0.9]] * 32, headless=True)
    env = SimpleNamespace(
        num_envs=32,
        device=torch.device("cpu"),
        progress_buf=torch.zeros(32, dtype=torch.long),
        reset_buf=torch.ones(32, dtype=torch.bool),
        terminate_buf=torch.zeros(32, dtype=torch.bool),
        simulator=simulator,
        robot_config=SimpleNamespace(anchor_body_index=0),
        motion_manager=SimpleNamespace(
            config=SimpleNamespace(init_start_prob=0.0),
            init_start_probs=torch.zeros(32),
        ),
        extras={},
    )
        ...
        command_source=RandomYawRateSteeringCommandSourceConfig(
            curriculum=[
                YawRateSteeringCurriculumStageConfig(
                    ...
                    moving_yaw_rate_probability=0.0,
                    init_start_prob=1.0,
                    advance_lin_vel_reward_threshold=0.8,
                    advance_yaw_rate_reward_threshold=0.8,
                ),
                YawRateSteeringCurriculumStageConfig(
                    start_epoch=5,
                    tar_speed_min=0.2,
    ...
    assert control.command_source._active_stage_index == 0
    assert torch.all((control._tar_speed >= 0.3) & (control._tar_speed <= 0.7))
    assert torch.allclose(control._tar_local_vel[:, 1], torch.zeros(32))
    assert torch.equal(control._tar_yaw_rate, torch.zeros(32))
    assert env.motion_manager.config.init_start_prob == 1.0
    assert torch.equal(
        env.motion_manager.init_start_probs, torch.ones(32)
    )

    control.on_epoch_end(5)
    assert control.command_source._active_stage_index == 0
    control.command_source.record_tracking_rewards(
        torch.full((32,), 0.9),
        torch.full((32,), 0.9),
    )
    control.on_epoch_end(5)
    assert control.command_source._active_stage_index == 1
    assert torch.equal(control._tar_local_vel, torch.zeros(32, 2))
    assert torch.all(
        (torch.abs(control._tar_yaw_rate) >= 0.4)
        & (torch.abs(control._tar_yaw_rate) <= 0.6)
    )
    assert env.motion_manager.config.init_start_prob == 0.5
    assert torch.equal(
        env.motion_manager.init_start_probs, torch.full((32,), 0.5)
    )
    state_dict = control.get_state_dict()

    restored = SteeringControl(config, env)
    restored.load_state_dict(state_dict)
    assert restored.command_source._current_epoch == 5
    assert restored.command_source._active_stage_index == 1
