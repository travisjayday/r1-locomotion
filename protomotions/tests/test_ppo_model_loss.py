...
def _make_ppo_agent():
    ...
    agent.config = SimpleNamespace(
        ...
        entropy_coef_decay_epochs=None,
...
def test_l2c2_clean_forward_uses_unwrapped_actor_module():
    ...
    agent.config = SimpleNamespace(
        ...
        symmetry=SimpleNamespace(enabled=False),
    )
    batch_td = TensorDict(
        {
...
def test_entropy_coef_holds_flat_when_decay_unset():
    agent = _make_ppo_agent()
    agent.config.entropy_coef = 0.01
    agent.current_epoch = 12345
    assert agent._current_entropy_coef() == 0.01


def test_entropy_coef_anneals_linearly_then_holds_at_final():
    agent = _make_ppo_agent()
    agent.config.entropy_coef = 0.005
    agent.config.entropy_coef_final = 0.0
    agent.config.entropy_coef_decay_epochs = 8000

    agent.current_epoch = 0
    assert agent._current_entropy_coef() == pytest.approx(0.005)
    agent.current_epoch = 4000
    assert agent._current_entropy_coef() == pytest.approx(0.0025)
    agent.current_epoch = 8000
    assert agent._current_entropy_coef() == pytest.approx(0.0)
    # Holds at the final value rather than continuing past it or dividing by
    # a stale/zero denominator once training runs longer than the decay window.
    agent.current_epoch = 20000
    assert agent._current_entropy_coef() == pytest.approx(0.0)


def test_entropy_coef_anneals_toward_a_nonzero_floor():
    agent = _make_ppo_agent()
    agent.config.entropy_coef = 0.01
    agent.config.entropy_coef_final = 0.002
    agent.config.entropy_coef_decay_epochs = 1000

    agent.current_epoch = 500
    assert agent._current_entropy_coef() == pytest.approx(0.006)
