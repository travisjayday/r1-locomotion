...
def test_mujoco_key_callback_maps_space_and_allows_repeated_presses():
    pytest.importorskip("mujoco")
    from protomotions.simulator.mujoco.simulator import MujocoSimulator

    ui = UserInterface()
    stop = ui.register_key("SPACE", owner="steering", description="Stop")
    simulator = object.__new__(MujocoSimulator)
    simulator.user_interface = ui

    simulator._mujoco_key_callback(ord(" "))
    assert stop.consume() is True
    assert stop.down() is False

    ui.begin_step()
    simulator._mujoco_key_callback(ord(" "))
    assert stop.consume() is True

    # Other ASCII whitespace and special/non-printable codes are ignored.
    simulator._mujoco_key_callback(ord("\t"))
    simulator._mujoco_key_callback(127)
