...
class RobotAssetConfig:
    ...
    # Bodies whose colliders may participate in selected robot self-collisions,
    # but must not collide with terrain or other environment assets. MuJoCo can
    # express this directly with contype/conaffinity; Isaac Lab needs an
    # additional USD filtered-pairs pass because its MJCF importer drops those
    # general mask bits.
    environment_collision_exclusion_bodies: List[str] = field(default_factory=list)
...
class ControlConfig:
    ...
    # Fraction of the effort-limit-equivalent PD deflection represented by a
    # unit BeyondMimic action. Kept at 1.0 for existing robot configurations.
    action_effort_fraction: float = 1.0
