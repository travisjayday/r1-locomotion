...
class PerspectiveViewer(object):
    def __init__(self, resolution_scale: float = 1.0):
        ...
        # Kit persists this setting in its user config. Set it even at 1.0 so
        # a previous low-resolution benchmark cannot leak into later runs.
        self.viewport_api.set_texture_resolution_scale(resolution_scale)
        # Disable advanced rendering features
        self.disable_advanced_rendering()

    def close(self):
        # Do not leave an explicit low-resolution override in Kit's persistent
        # viewport settings for unrelated Isaac Sim sessions.
        self.viewport_api.set_texture_resolution_scale(1.0)
