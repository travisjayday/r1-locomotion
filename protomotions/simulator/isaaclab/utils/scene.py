...
from protomotions.simulator.isaaclab.utils.unitree_actuator import UnitreeActuatorCfg
    ...
    def __init__(
        ...
        **kwargs,
        ...
        for actuator_group in actuator_groups:
            GroupActuatorConfig = (
                UnitreeActuatorCfg
                if actuator_group.actuator_model == "unitree"
                else ActuatorConfig
            )
            actuator_params = dict(actuator_group.params)
            if actuator_group.actuator_model == "unitree":
                actuator_dr = (
                    config.domain_randomization.actuator
                    if config.domain_randomization is not None
                    else None
                )
                if actuator_dr is not None:
                    # Allocate the delay history for the final curriculum
                    # envelope. The active stage sets the sampled lag later.
                    actuator_params["min_delay"] = 0
                    actuator_params["max_delay"] = (
                        actuator_dr.delay_steps_range[1]
                    )
            actuators[actuator_group.name] = GroupActuatorConfig(
