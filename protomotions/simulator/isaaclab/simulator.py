...
from protomotions.simulator.isaaclab.utils.collision_filtering import (
    apply_body_environment_collision_filters,
)
...
class IsaacLabSimulator(Simulator):
    ...
    def __init__(
        ...
        sim_cfg = sim_utils.SimulationCfg(
            ...
            gravity=getattr(self.config.sim, "gravity", (0.0, 0.0, -9.81)),
        ...
        if not self.headless:
            ...
            self.camera_follow_enabled = not getattr(
                self.config, "disable_camera_follow", False
            )
    ...
    def _create_simulation(self) -> None:
        ...
        excluded_environment_bodies = getattr(
            self.robot_config.asset,
            "environment_collision_exclusion_bodies",
            (),
        )
        if excluded_environment_bodies:
            filtered_count = apply_body_environment_collision_filters(
                self._scene.stage, excluded_environment_bodies
            )
            expected_count = self.num_envs * len(excluded_environment_bodies)
            if filtered_count != expected_count:
                raise RuntimeError(
                    "Failed to apply environment collision filters to every selected "
                    f"robot body: updated {filtered_count}, expected {expected_count}."
                )
        self._sim.reset()
    ...
    def _apply_domain_randomization_if_needed(self) -> None:
        """Apply the active DR stage from nominal robot properties."""
        # PhysX material/COM tensor setters require CPU index buffers even
        # when the property tensor itself is exposed through Warp on CUDA.
        # Articulation mass/inertia and actuator tensors, by contrast, are
        # indexed on the simulator device.
        all_env_ids = torch.arange(
            self.config.num_envs, dtype=torch.int32, device="cpu"
        )
        device_env_ids = torch.arange(
            self.config.num_envs, dtype=torch.int32, device=self.device
        )
        if not hasattr(self, "_dr_default_materials"):
            self._dr_default_materials = wp.to_torch(
        ...
        if (
            ...
            if not hasattr(self, "_dr_default_coms"):
                self._dr_default_coms = wp.to_torch(
                    self._robot.root_view.get_coms()
                ).clone()
        ...
        self._apply_rigid_body_domain_randomization(device_env_ids)
        self._apply_actuator_domain_randomization(device_env_ids)

        self._apply_scene_object_properties_after_spawn(all_env_ids)

    def _apply_robot_domain_randomization(self) -> None:
        """Reapply robot properties when a curriculum stage changes."""
        self._apply_domain_randomization_if_needed()

    def _apply_rigid_body_domain_randomization(
        self, all_env_ids: torch.Tensor
    ) -> None:
        rigid_body_dr = (
            self._domain_randomization.get("rigid_body")
            if self._domain_randomization is not None
            else None
        )
        if rigid_body_dr is None:
            return

        if not hasattr(self, "_dr_default_body_masses"):
            self._dr_default_body_masses = self._robot.data.body_mass.torch.clone()
            self._dr_default_body_inertias = (
                self._robot.data.body_inertia.torch.clone()
            )

        common_body_ids = rigid_body_dr["body_indices"]
        body_names = [self._body_names[index] for index in common_body_ids]
        backend_body_ids, _ = self._robot.find_bodies(
            body_names, preserve_order=True
        )
        body_ids = torch.as_tensor(
            backend_body_ids, dtype=torch.int32, device=self.device
        )
        mass_scale = rigid_body_dr["mass_scale"].to(self.device)
        inertia_scale = rigid_body_dr["inertia_scale"].to(self.device)

        default_masses = self._dr_default_body_masses[
            all_env_ids[:, None], body_ids
        ]
        masses = default_masses * mass_scale
        ...
        # Preserve the nominal shape-derived inertia/mass relationship, then
        # add a smaller independent inertia uncertainty.
        default_inertias = self._dr_default_body_inertias[
            all_env_ids[:, None], body_ids
        ]
        inertias = default_inertias * mass_scale.unsqueeze(-1)
        inertias = inertias * inertia_scale.unsqueeze(-1)
    ...
    def _apply_actuator_domain_randomization(
        self, all_env_ids: torch.Tensor
    ) -> None:
        actuator_dr = (
            self._domain_randomization.get("actuator")
            if self._domain_randomization is not None
            else None
        )
        ...
        common_dof_ids = actuator_dr["dof_indices"]
        selected_scales = {
            common_id: column for column, common_id in enumerate(common_dof_ids)
        }
        joint_names = build_isaaclab_joint_name_map(
            self.robot_config.kinematic_info
        )
        backend_to_common = {
            self._robot.joint_names.index(
                joint_names.semantic_to_backend[self._dof_names[common_id]]
            ): common_id
            for common_id in common_dof_ids
        }

        if not hasattr(self, "_dr_default_actuator_properties"):
            self._dr_default_actuator_properties = {
                name: {
                    "stiffness": actuator.stiffness.clone(),
                    "damping": actuator.damping.clone(),
                    "motor_strength": (
                        actuator.motor_strength_scale.clone()
        ...
        delay_steps = actuator_dr["delay_steps"].to(
            device=self.device, dtype=torch.int
        )
        delay_min, delay_max = actuator_dr["delay_steps_range"]
        for name, actuator in self._robot.actuators.items():
            default = self._dr_default_actuator_properties[name]
            global_joint_ids = actuator.joint_indices
            if isinstance(global_joint_ids, slice):
                global_joint_ids = torch.arange(
                    self._robot.num_joints, device=self.device
                )[global_joint_ids]
            else:
                global_joint_ids = torch.as_tensor(
                    global_joint_ids, device=self.device
                )

            stiffness = default["stiffness"].clone()
            damping = default["damping"].clone()
            motor_strength = (
                default["motor_strength"].clone()
                if default["motor_strength"] is not None
                else None
            )
            changed_local_ids = []
            for local_id, global_id_tensor in enumerate(global_joint_ids):
                global_id = int(global_id_tensor)
                common_id = backend_to_common.get(global_id)
                if common_id is None:
                    continue
                column = selected_scales[common_id]
                stiffness[:, local_id] *= actuator_dr["stiffness_scale"][:, column]
                damping[:, local_id] *= actuator_dr["damping_scale"][:, column]
                if motor_strength is not None:
                    motor_strength[:, local_id] *= actuator_dr[
                        "motor_strength_scale"
                    ][:, column]
                changed_local_ids.append(local_id)

            if changed_local_ids:
                actuator.stiffness[:] = stiffness
                actuator.damping[:] = damping
                if motor_strength is not None:
                    actuator.motor_strength_scale[:] = motor_strength

            # UnitreeActuator inherits DelayedPDActuator. Updating cfg keeps
            # future reset-time samples inside the active stage, while setting
            # the buffers now makes the epoch transition immediate.
            if hasattr(actuator, "positions_delay_buffer"):
                actuator.cfg.min_delay = delay_min
                actuator.cfg.max_delay = delay_max
                for buffer_name in (
                    "positions_delay_buffer",
                    "velocities_delay_buffer",
                    "efforts_delay_buffer",
                ):
                    getattr(actuator, buffer_name).set_time_lag(
                        delay_steps, all_env_ids
                    )
    ...
    def _physics_step(self) -> None:
        ...
        for idx in range(self.decimation):
            ...
            if self._physics_substep_callback is not None:
                self._physics_substep_callback(
                    idx + 1, self._sim.get_physics_dt()
                )
        ...
        if not self.headless and not self._perspective_view_failed:
            try:
                if not hasattr(self, "_perspective_view"):
                    ...
                    self._init_camera()
                elif self.camera_follow_enabled:
                    self._update_camera()
            except Exception as e:
                log.warning(
    ...
    def _get_root_pos_for_camera(self, env_id: int) -> np.ndarray:
        """
        Fetch only the robot root position, for camera tracking.

        Camera tracking only needs a 3-vector, but `_get_simulator_root_state`
        fetches and clones position, rotation, linear velocity, and angular
        velocity together. Fetching just the position tensor skips three
        unnecessary device-side conversions. The `.clone()` those getters use
        (to snapshot data before the next physics step overwrites Warp's
        underlying buffer, since `wp.to_torch` is zero-copy) is also
        unnecessary here: `.cpu()` already materializes an independent host
        copy, so no separate clone is needed for a value that is consumed
        immediately and not held across a step.
        """
        root_pos_w = wp.to_torch(self._robot.data.root_pos_w)
        return root_pos_w[env_id].cpu().numpy()

    def _get_object_root_pos_for_camera(self, env_id: int, obj_idx: int) -> np.ndarray:
        """
        Fetch only one scene object's root position, for camera tracking.

        Unlike `_get_simulator_object_root_state`, this does not loop over
        and stack every scene object's full state just to read one object's
        position.
        """
        root_pos_w = wp.to_torch(self._object[obj_idx].data.root_pos_w)
        return root_pos_w[env_id].cpu().numpy()
    ...
    def _init_camera(self) -> None:
        ...
        """
        self._cam_prev_char_pos = self._get_root_pos_for_camera(0)
    ...
    def _update_camera(self) -> None:
        ...
        if self._camera_target["element"] == 0:
            char_root_pos = self._get_root_pos_for_camera(
                self._camera_target["env"]
            )
            height_offset = 0.2
        else:
            in_scene_object_id = self._camera_target["element"] - 1
            char_root_pos = self._get_object_root_pos_for_camera(
                self._camera_target["env"], in_scene_object_id
            )
            height_offset = 0
    ...
    def close(self) -> None:
        ...
        """
        perspective_view = getattr(self, "_perspective_view", None)
        if perspective_view is not None:
            perspective_view.close()
        super().close()
