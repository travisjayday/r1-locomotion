...
class ActuatorDomainRandomizationConfig:
    """Multiplicative actuator uncertainty and command latency.

    Scale ranges are relative to the configured nominal actuator. Delay is in
    physics substeps, not policy steps; at 200 Hz, one step is 5 ms.
    """

    stiffness_scale_range: Tuple[float, float] = (1.0, 1.0)
    damping_scale_range: Tuple[float, float] = (1.0, 1.0)
    motor_strength_scale_range: Tuple[float, float] = (1.0, 1.0)
    ...
    def __post_init__(self):
        if self.dof_names is not None and self.dof_indices is not None:
            raise ValueError("Only one of dof_names or dof_indices may be provided.")
        if self.dof_names is None and self.dof_indices is None:
            raise ValueError("Either dof_names or dof_indices must be provided.")
        for name in (
            ...
            value_range = getattr(self, name)
            if (
                len(value_range) != 2
                or value_range[0] <= 0.0
                or value_range[0] > value_range[1]
            ):
                raise ValueError(f"{name} must be a positive (min, max) range.")
        if (
            len(self.delay_steps_range) != 2
            or any(not isinstance(value, int) for value in self.delay_steps_range)
            or self.delay_steps_range[0] < 0
            or self.delay_steps_range[0] > self.delay_steps_range[1]
        ):
            raise ValueError(
                "delay_steps_range must be a non-negative integer (min, max) range."
            )


@dataclass
class RigidBodyDomainRandomizationConfig:
    """Multiplicative robot link mass and inertia uncertainty."""

    mass_scale_range: Tuple[float, float] = (1.0, 1.0)
    inertia_scale_range: Tuple[float, float] = (1.0, 1.0)
    body_names: Optional[List[str]] = None
    body_indices: Optional[List[int]] = None

    def __post_init__(self):
        if self.body_names is not None and self.body_indices is not None:
            raise ValueError("Only one of body_names or body_indices may be provided.")
        if self.body_names is None and self.body_indices is None:
            raise ValueError("Either body_names or body_indices must be provided.")
        for name in ("mass_scale_range", "inertia_scale_range"):
            value_range = getattr(self, name)
            if (
                len(value_range) != 2
                or value_range[0] <= 0.0
...
@dataclass
class DomainRandomizationCurriculumStageConfig:
    """Activate a fraction of the configured final randomization envelope."""
    ...
    def __post_init__(self):
        if self.start_epoch < 0:
            raise ValueError("Domain-randomization stage start_epoch must be non-negative.")
        if not 0.0 <= self.intensity <= 1.0:
            raise ValueError("Domain-randomization stage intensity must be in [0, 1].")
...
class DomainRandomizationConfig:
    ...
    actuator: Optional[ActuatorDomainRandomizationConfig] = field(
        default=None,
        metadata={"help": "Actuator gains, strength, and latency randomization."},
    )
    rigid_body: Optional[RigidBodyDomainRandomizationConfig] = field(
        default=None,
        metadata={"help": "Robot link mass and inertia randomization."},
    )
    curriculum: Optional[List[DomainRandomizationCurriculumStageConfig]] = field(
        default=None,
        metadata={
            "help": (
                "Epoch stages that scale all configured randomization ranges "
                "from nominal (intensity=0) to their final envelope (intensity=1)."
            )
        },
    )
    ...
    def __post_init__(self):
        ...
        if not self.curriculum:
            raise ValueError("Domain-randomization curriculum must not be empty.")
        starts = [stage.start_epoch for stage in self.curriculum]
        if starts[0] != 0:
            raise ValueError("Domain-randomization curriculum must start at epoch 0.")
        if starts != sorted(set(starts)):
            raise ValueError(
                "Domain-randomization curriculum stages need unique, increasing epochs."
            )

    def stage_index(self, current_epoch: int) -> int:
        """Return the active curriculum stage for an absolute run epoch."""
        if not self.curriculum:
            return 0
        active = 0
        for index, stage in enumerate(self.curriculum):
            if current_epoch < stage.start_epoch:
                break
            active = index
        return active

    def resolved_for_epoch(self, current_epoch: int) -> "DomainRandomizationConfig":
        """Return a copy with all uncertainty envelopes scaled for the epoch."""
        if not self.curriculum:
            return self
        intensity = self.curriculum[self.stage_index(current_epoch)].intensity
        resolved = deepcopy(self)
        resolved.curriculum = None

        def scaled_range(value_range, nominal):
            if value_range is None:
                return None
            return tuple(
                nominal + (float(value) - nominal) * intensity
                for value in value_range
            )

        if resolved.action_noise is not None:
            resolved.action_noise.action_noise_range = scaled_range(
                resolved.action_noise.action_noise_range, 0.0
            )
        if resolved.friction is not None:
            resolved.friction.static_friction_range = scaled_range(
                resolved.friction.static_friction_range, 1.0
            )
            resolved.friction.dynamic_friction_range = scaled_range(
                resolved.friction.dynamic_friction_range, 1.0
            )
            resolved.friction.restitution_range = scaled_range(
                resolved.friction.restitution_range, 0.0
            )
        if resolved.center_of_mass is not None:
            resolved.center_of_mass.com_range = {
                axis: scaled_range(value_range, 0.0)
                for axis, value_range in resolved.center_of_mass.com_range.items()
            }
        if resolved.observation_noise is not None:
            for noise_field in fields(resolved.observation_noise):
                value = getattr(resolved.observation_noise, noise_field.name)
                if isinstance(value, list):
                    value = [float(item) * intensity for item in value]
                else:
                    value = float(value) * intensity
                setattr(resolved.observation_noise, noise_field.name, value)
        if resolved.push is not None:
            resolved.push.max_linear_velocity = tuple(
                float(value) * intensity
                for value in resolved.push.max_linear_velocity
            )
            resolved.push.max_angular_velocity = tuple(
                float(value) * intensity
                for value in resolved.push.max_angular_velocity
            )
        if resolved.actuator is not None:
            resolved.actuator.stiffness_scale_range = scaled_range(
                resolved.actuator.stiffness_scale_range, 1.0
            )
            resolved.actuator.damping_scale_range = scaled_range(
                resolved.actuator.damping_scale_range, 1.0
            )
            resolved.actuator.motor_strength_scale_range = scaled_range(
                resolved.actuator.motor_strength_scale_range, 1.0
            )
            low, high = resolved.actuator.delay_steps_range
            resolved.actuator.delay_steps_range = (
                int(low * intensity),
                int(high * intensity),
            )
        if resolved.rigid_body is not None:
            resolved.rigid_body.mass_scale_range = scaled_range(
                resolved.rigid_body.mass_scale_range, 1.0
            )
            resolved.rigid_body.inertia_scale_range = scaled_range(
                resolved.rigid_body.inertia_scale_range, 1.0
            )
        return resolved
