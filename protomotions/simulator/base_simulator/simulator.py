...
class Simulator(RecordingMixin, ABC):
    ...
    def __init__(
        ...
        # Optional evaluation-only observer. Backends that support substep
        # capture invoke this after refreshing their physics state tensors.
        self._physics_substep_callback = None
        ...
        self._domain_randomization_config = self.config.domain_randomization
        self._domain_randomization_stage_index = 0
        if self._domain_randomization_config is not None:
            self._domain_randomization_stage_index = (
                self._domain_randomization_config.stage_index(0)
            )
            self._active_domain_randomization_config = (
                self._domain_randomization_config.resolved_for_epoch(0)
            )
        else:
            self._active_domain_randomization_config = None
        initial_intensity = 0.0
        if (
            self._domain_randomization_config is not None
            and self._domain_randomization_config.curriculum
        ):
            initial_intensity = self._domain_randomization_config.curriculum[
                self._domain_randomization_stage_index
            ].intensity
        elif self._domain_randomization_config is not None:
            initial_intensity = 1.0
        self._domain_randomization_stage_metric = torch.tensor(
            float(self._domain_randomization_stage_index), device=self.device
        )
        self._domain_randomization_intensity_metric = torch.tensor(
            initial_intensity, device=self.device
        )
    ...
    def _init_push_randomization(self) -> None:
        ...
        self._configure_active_push_randomization()

    def _configure_active_push_randomization(self) -> None:
        """Refresh push parameters after a domain-randomization stage change."""
        push_cfg = None
        if (
            self._active_domain_randomization_config is not None
            and self._active_domain_randomization_config.push is not None
            and self._active_domain_randomization_config.push.has_push()
        ):
            push_cfg = self._active_domain_randomization_config.push
    ...
    @property
    def active_domain_randomization_config(self):
        """Resolved randomization envelope for the current training stage."""
        return self._active_domain_randomization_config

    def on_epoch_end(self, current_epoch: int) -> None:
        """Advance and apply the domain-randomization curriculum."""
        source = self._domain_randomization_config
        if source is None or not source.curriculum:
            return
        stage_index = source.stage_index(current_epoch)
        if stage_index == self._domain_randomization_stage_index:
            return

        self._domain_randomization_stage_index = stage_index
        self._domain_randomization_stage_metric.fill_(float(stage_index))
        self._domain_randomization_intensity_metric.fill_(
            source.curriculum[stage_index].intensity
        )
        self._active_domain_randomization_config = source.resolved_for_epoch(
            current_epoch
        )
        self._domain_randomization = self._process_domain_randomization(
            self._active_domain_randomization_config
        )
        self._configure_active_push_randomization()
        self._apply_robot_domain_randomization()
        stage = source.curriculum[stage_index]
        log.info(
            "Domain-randomization curriculum advanced to stage %d at epoch %d "
            "(intensity=%.3f)",
            stage_index,
            current_epoch,
            stage.intensity,
    ...
    def _apply_robot_domain_randomization(self) -> None:
        """Apply sampled robot properties; simulator backends may override."""

    def set_physics_substep_callback(
        self, callback: Optional[Callable[[int, float], None]]
    ) -> None:
        """Install an observer called after individual physics substeps.

        This is intended for high-rate trajectory capture while keeping the
        policy/control rate unchanged. ``None`` disables observation.
        """
        self._physics_substep_callback = callback
    ...
    def _process_domain_randomization(self, config=_ACTIVE_DR_CONFIG) -> None:
        """
        Process domain randomization from the config.
        """
        if config is _ACTIVE_DR_CONFIG:
            config = getattr(
                self,
                "_active_domain_randomization_config",
        ...
        if config.center_of_mass is not None:
            domain_randomization_dict["center_of_mass"] = (
                self._process_center_of_mass_domain_randomization(
                    config.center_of_mass
                )
            )
        if config.object_assets is not None:
            domain_randomization_dict["object_assets"] = (
                self._process_object_asset_domain_randomization(
                    config.object_assets
                )
            )
        if config.actuator is not None:
            domain_randomization_dict["actuator"] = (
                self._process_actuator_domain_randomization(config.actuator)
            )
        if config.rigid_body is not None:
            domain_randomization_dict["rigid_body"] = (
                self._process_rigid_body_domain_randomization(config.rigid_body)
            )
    ...
    def _process_actuator_domain_randomization(
        ...
        def sample(value_range):
            return (
                torch.rand(self.num_envs, num_dofs, device=self.device)
                * (value_range[1] - value_range[0])
                + value_range[0]
            )

        delay_min, delay_max = domain_randomization.delay_steps_range
        ...
        return {
            ...
            "stiffness_scale": sample(
                domain_randomization.stiffness_scale_range
            ),
            "damping_scale": sample(domain_randomization.damping_scale_range),
            "motor_strength_scale": sample(
                domain_randomization.motor_strength_scale_range
            ),
            "delay_steps": delay_steps,
            "delay_steps_range": domain_randomization.delay_steps_range,
        }

    def _process_rigid_body_domain_randomization(
        self, domain_randomization: RigidBodyDomainRandomizationConfig
    ) -> Dict[str, Any]:
        ...
        def sample(value_range):
            return (
                torch.rand(self.num_envs, num_bodies, device=self.device)
                * (value_range[1] - value_range[0])
                + value_range[0]
            )
        ...
        return {
            ...
            "mass_scale": sample(domain_randomization.mass_scale_range),
            "inertia_scale": sample(domain_randomization.inertia_scale_range),
        }
