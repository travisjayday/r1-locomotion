...
class MujocoSimulator(Simulator):
    ...
    def __init__(
        ...
        self._viewer_pacer = (
            RealTimePacer(
                real_time_factor=getattr(config, "viewer_real_time_factor", 1.0),
                max_lag_s=getattr(config, "viewer_max_lag_s", 0.1),
            )
            if getattr(config, "sync_viewer_to_real_time", True)
            else None
        )
        ...
        self._actuator_translation = None
        self._uses_explicit_actuator_translation = False
    ...
    def _add_ground_and_light(root: ET.Element) -> None:
        ...
        # Check if a ground/floor geom already exists AND actually collides.
        # Some MJCFs (e.g. r1.xml) ship a purely decorative floor geom with
        # contype="0" conaffinity="0" -- a "reference grid" meant for
        # simulators (IsaacLab/PhysX) that provide their own terrain
        # collision separately. Matching only on name/type here would leave
        # the standalone MuJoCo sim with no real ground and the robot falls
        # straight through it.
        has_ground = False
        for geom in worldbody.findall("geom"):
            ...
            if "floor" in name or "ground" in name or geom_type == "plane":
                if geom.get("contype", "1") == "0" and geom.get("conaffinity", "1") == "0":
                    # Decorative-only: give it real collision instead of
                    # adding a second overlapping plane.
                    geom.set("contype", "1")
                    geom.set("conaffinity", "1")
                    print(f"  Enabled collision on decorative ground geom '{name}'")
                has_ground = True
                break
    ...
    def _create_simulation(self) -> None:
        ...
        # Normalize armature, passive friction, and redundant force caps
        ...
        # Configure actuators based on PD mode. Custom actuator descriptions
        # cannot be represented by MuJoCo position actuators, so they always
        # use the explicit translation layer even when use_implicit_pd=True.
        from protomotions.robot_configs.base import ControlType

        use_implicit_pd = getattr(self.config, "use_implicit_pd", True)
        has_custom_actuators = (
            self._actuator_translation is not None
            and self._actuator_translation.has_custom_models
        )
        self._uses_explicit_actuator_translation = (
            self.control_type == ControlType.BUILT_IN_PD
            and (has_custom_actuators or not use_implicit_pd)
        )
        if self._uses_explicit_actuator_translation:
            if has_custom_actuators:
                print(
                    "  PD mode: EXPLICIT actuator translation "
                    "(custom robot actuator model)"
                )
            else:
                print("  PD mode: EXPLICIT (manual PD at each physics substep)")
            self._configure_explicit_pd()
        elif use_implicit_pd:
            # Convert motor actuators to position actuators (MuJoCo handles PD internally)
            self._configure_actuators_for_pd()
        else:
            # Non-PD control modes keep their original motor actuators.
            self._configure_explicit_pd()
    ...
    def _override_joint_properties(self) -> None:
        """Normalize joint properties for simulator-independent actuation.
        ...
        MuJoCo ``frictionloss`` is a constant torque and is not equivalent to
        IsaacLab's joint-friction coefficient. Custom motor friction is applied
        by :class:`ActuatorModelTranslation`, so residual MJCF frictionloss must
        not be double-counted. Joint-level actuator force caps are disabled in
        favor of the robot configuration's per-actuator effort limits.
        """
        ...
        for i in range(self.model.njnt):
            ...
            if jnt_name in control_info:
                ...
                old_frictionloss = self.model.dof_frictionloss[dof_addr]
                ...
                self.model.jnt_actfrclimited[i] = 0

                print(
                    f"  Joint '{jnt_name}' DOF[{dof_idx}]: "
                    f"armature {old_armature:.6f} -> {self.model.dof_armature[dof_addr]:.6f}, "
                    f"frictionloss {old_frictionloss:.6f} -> 0.0, "
                    "joint force cap -> actuator limit"
                )
    ...
    def _setup_control_parameters(self) -> None:
        ...
        sim_dof_names = self._get_sim_body_ordering().dof_names
        self._actuator_translation = ActuatorModelTranslation(
            control_info, sim_dof_names
        )
    ...
    def _configure_explicit_pd(self) -> None:
        """Set up explicit torque control on the compiled MuJoCo actuators.

        The source MJCF normally contains motor actuators already, but reset all
        gain/bias parameters here so switching control modes cannot leave an
        implicit position-actuator transform behind.
        """
        control_info = self.robot_config.control.control_info
        ...
        for act_idx in range(self.model.nu):
            jnt_id = self.model.actuator_trnid[act_idx, 0]
            jnt_name = mujoco.mj_id2name(self.model, mujoco.mjtObj.mjOBJ_JOINT, jnt_id)
            self.model.actuator_gaintype[act_idx] = 0  # mjGAIN_FIXED
            self.model.actuator_gainprm[act_idx, :] = 0.0
            self.model.actuator_gainprm[act_idx, 0] = 1.0
            self.model.actuator_biastype[act_idx] = 0  # mjBIAS_NONE
            self.model.actuator_biasprm[act_idx, :] = 0.0
    ...
    def _recompute_explicit_pd(self) -> None:
        ...
        if self._actuator_translation is not None:
            torques = self._actuator_translation.compute(
                self._pd_targets_sim, q, qd
            )
        else:
            torques = self._kp_sim * (self._pd_targets_sim - q) - self._kd_sim * qd
            torques = np.clip(
                torques, -self._effort_limits_sim, self._effort_limits_sim
            )
    ...
    def _init_viewer(self) -> None:
        ...
        if self._viewer_pacer is not None:
            self._viewer_pacer.reset(self.data.time)
    ...
    def _physics_step(self) -> None:
        ...
        Two PD modes for BUILT_IN_PD:
          - Implicit: MuJoCo position actuators handle PD internally each substep.
          - Explicit: We recompute translated actuator torques at each substep.

        A custom robot actuator model selects explicit mode automatically;
        otherwise ``config.use_implicit_pd`` controls the selection.
        ...
        use_explicit_substep_pd = self._uses_explicit_actuator_translation
        ...
        if self._has_free_joint:
            ...
            dof_start = 0
        self._last_applied_torques = self.data.qfrc_actuator[
            dof_start : dof_start + self._num_actuated_dofs
        ].copy()
        self._step_count += 1
    ...
    def _get_simulator_root_state(
        ...
        if self._has_free_joint:
            ...
            root_velocity = self._get_body_velocity(1)
            root_ang_vel = root_velocity[0:3]
            root_vel = root_velocity[3:6]
        else:
    ...
    def _get_body_velocity(self, body_id: int) -> np.ndarray:
        """Return object-centered body velocity in world orientation (rot:lin)."""
    ...
    def _get_simulator_bodies_state(
        ...
        # data.cvel is centered at MuJoCo's subtree COM and is therefore not
        # directly comparable to IsaacLab's velocity at each body origin.
        body_velocities = np.stack(
            [self._get_body_velocity(body_id) for body_id in range(1, 1 + nb)]
        )
        body_ang_vel = body_velocities[:, 0:3]
        body_vel = body_velocities[:, 3:6]
    ...
    def _apply_simulator_pd_targets(self, pd_targets: torch.Tensor) -> None:
        ...
        Two modes:
          - Implicit: Write position targets to data.ctrl. MuJoCo position
            actuators compute PD internally each substep.
          - Explicit/custom: Cache targets. Translated actuator torques are
            recomputed at each substep in _physics_step.
        """
        ...
        if not self._uses_explicit_actuator_translation:
    ...
    def _mujoco_key_callback(self, keycode: int) -> None:
        """Handle keyboard presses from the MuJoCo passive viewer.

        MuJoCo special keys use non-ASCII key codes that do not correspond to
        the string keys registered in ``UserInterface``. Space is reported as
        the literal ASCII whitespace character, so translate it to the shared
        registry's ``SPACE`` name. The callback exposes presses but no releases;
        emit a complete pulse so the same key can be pressed again later.
        """
        if keycode == ord(" "):
            key = "SPACE"
        elif keycode < 33 or keycode > 126:
            return
        else:
            key = chr(keycode)
        self.user_interface.handle_key_event(key, pressed=True)
        self.user_interface.handle_key_event(key, pressed=False)
    ...
    def render(self) -> None:
        ...
        if not self.headless and self.viewer is not None and self._viewer_initialized:
            if not self.viewer.is_running():
                ...
                return
            if self._viewer_pacer is not None:
                self._viewer_pacer.wait(self.data.time)
